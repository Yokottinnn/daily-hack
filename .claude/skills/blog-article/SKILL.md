...
### バナーを避ける条件を作りすぎると、本命を捨てる

**「バナーを弾く」は、やりすぎると施設写真そのものを落とす。**

2026-08-30 のららぽーと記事で、公式の施設ページから写真を 63 枚取ったところ
**1 枚も施設写真が無かった。** 取れていたのは学校の集合写真、太陽光の相談会チラシ、
パイ菓子の商品写真、テナントのロゴ。

原因は自分で書いた除外条件。

```python
BAD = re.compile(r"logo|icon|...|kv|main_?visual|common|header|footer")  # ← やりすぎ
```

**施設ページのキービジュアルこそが施設写真**なのに、`kv` と `main_visual` で
それを落とし、残ったイベント一覧・ショップニュースの画像を拾っていた。

**ハズレの原因は、ファイル名ではなく「どの階層の画像か」だった。**

```python
BAD      = r"logo|icon|favicon|sprite|placeholder|noimage|arrow|btn|sns"  # ロゴ類だけ
BAD_PATH = r"/(event|shopnews|pickup|topics|openrenewal)/"                # 階層で弾く
```

**og:image は候補に入れてよい。** 「og:image は SNS 用のブランド画像」は
**施設一覧のタイル**を作るときの話で、**個別ページのヒーロー**を探すときは逆に本命に近い。

判断の順は次のとおり。

1. **まず数枚だけ取って目で見る。** 21 施設 × 3 枚を一気に取る前に、1 施設で確かめる
2. ハズレていたら、**弾き方を足すのではなく、外れている理由を先に特定する**
3. 公式で取れなければ**プレスリリース**、それでも無ければ **Wikimedia Commons**（下記）

### 公式から取れないときは、プレスリリースを見る

**公式サイトに載っていない金額と写真が、その会社のプレスリリースには載っている。**
PR TIMES は**会社が自分で出した一次情報**なので、出典を書けば根拠として使える。

2026-08-30 のサウナ記事で、公式サイトを 2 周しても埋まらなかったものが、
**リリースを見に行った 1 周で埋まった。**

| 公式サイトでは | リリースでは |
| --- | --- |
| SHIAGARU は「60分／90分／120分」の区分だけ。**金額は予約カレンダーの中** | **60分1,600円〜／90分2,000円〜／120分2,300円〜**が但し書きごと出ていた |
| 荒木町Logout・黄金湯新宿・門仲SAUNAS LO は写真が拾えない | **どれも施設写真が4枚以上**載っていた |
| 門仲SAUNAS LO は「この夏オープン予定」のティザー図だけ | **実際のサウナ室の写真**があった |

```text
公式サイト → 取れなければ プレスリリース → それでも無ければ「取れなかった」と書く
```

**リリースの所在はクラウド側の検索で確定させてから渡す。**
`prtimes.jp/main/html/rd/p/<8桁>.<9桁>.html` の形。**番号を当て推量で組み立てない。**

画像は `prcdn.freetls.fastly.net/release_image/...` にあり、そのまま取得できる。
**それでも中身は目で見る。** 同じリリースの中に別店舗の写真が混ざっていることがある
（SHIAGARU の公式サイトでは、1 枚目が**福岡・天神店**の写真だった）。
...
## 2-B. 題材を選ぶ基準｜「数字の賞味期限」を見る

**書く前に、その題材の数字が何か月もつかを考える。** 2026-08-30 に
「回転寿司チェーンの店舗数ランキング」を提案して、利用者から差し戻された。

> 回転寿司は比較しづらいし値段がしょっちゅう変わるからどうだろうか

**両方とも正しく、実測とも一致していた。**

| 数字の種類 | 賞味期限 | このブログでの実測 |
| --- | --- | --- |
| 面積・店舗数・開業年・決算の売上 | **年単位** | ららぽーと記事＝表示413（サイト全体の67%） |
| 価格・キャンペーン・時限の割引 | **月単位** | クレカ／銀行キャンペーン系＝**表示1〜3で沈んでいる** |

`credit-card-campaign-2026-07` / `june-2026-campaigns-roundup` /
`may-2026-bank-campaigns-roundup` は、いずれも短命な数字の記事で、
**45 記事のなかで軒並み伸びていない。**

### 比較の軸が何本あるかも見る

**軸が 1 本しかない題材は、比較記事として弱い。**

| | 軸 | 判定 |
| --- | --- | --- |
| ららぽーと | 面積／店舗数／売上（**定義が揺れない 3 本**） | 成立した |
| 回転寿司 | 「1皿◯円」だけ。しかも皿の中身が各社で違う | **成立しない** |

ネタもサイズも違うものを「1皿の値段」で並べても、**公平な比較にならない。**

### 短命な数字を扱うときの逃げ方

価格が主題の記事を書くなら、**軸を「いくら」から動かす。**

- ✅ **「何時まで」「何がついてくるか」**（提供時間・セット内容は価格より安定する）
- ✅ 仕組みそのもの（コメダの「ドリンク代のみでトースト＋ゆで卵」は長年変わっていない）
- ✗ 価格の細部を主題にする

**価格は表に入れてよいが、記事の主題にはしない。** そのうえで
「料金は改定される。最新は公式で確認すること」を必ず書く。
