...
**タイルには札を出す。** `img6` のうしろにラベルを 6 つ渡すと、各タイルの左下に
名前が入る。**写真だけでは「どこの店か」が伝わらない。**

```bash
python3 scripts/gen-tile6-hero.py OUT.jpg KICKER T1 T2 SUB CREDIT img1 … img6 \
    "コメダ珈琲店" "すき家" "松屋" "吉野家" "ドトール" "上島珈琲店"
```

**見出しは帯の幅に自動で収まる**（2026-09-04 に対応）。それ以前は 96px 固定で、
**8 文字の見出しがタイル側にはみ出して読めなくなっていた。** 生成後に必ず目で見て、
見出しが帯の中で完結しているかを確かめる。
