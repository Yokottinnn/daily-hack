---
name: blog-article
description: daily-hack のブログ記事を書く・直す・公開するときの必須手順。記事の新規作成、既存記事の加筆・リライト、アイキャッチや記事内画像の用意、公開前の検証で必ず使う。「記事を書いて」「〇〇のまとめを作って」「この投稿を参考にして記事を」「表紙を作って」「記事を直して」に該当したら、本文を1行でも書く前に読むこと。
---

# ブログ記事の作り方（daily-hack）

**このスキルを読まずに記事を書き始めない。** 2026-08-22 に、読まずに書いた記事が
**サウナと無関係の表紙**（カテゴリ共通フォールバック画像）で公開され、差し戻された。
既存のパイプライン（`scripts/gen-*-eyecatch.py`・`fetch-commons-photo.py`・
`check-article-ux.py`）を無視したのが原因。

基準は直近の高品質 3 本。**構成・部品・検証をこの 3 本に合わせる。**

- `src/content/posts/lalaport-guide-2026.md`（表示 338・平均 7.7 位。最も検索に効いている）
- `src/content/posts/outlet-mall-guide-2026.md`
- `src/content/posts/wangan-august-events-2026.md`

迷ったら**この 3 本を開いて真似する。** 記憶で書かない。

## 0. 絶対に外さない 10 個

| # | ルール | 破ると |
| --- | --- | --- |
| 1 | **アイキャッチを必ず作る**（`/images/<slug>/eyecatch.jpg`・1600×900） | 記事と無関係の共通画像が表紙になる |
| 2 | **1 セクション 1 ビジュアル**（h2/h3 ごとに画像・表・X 埋め込みのどれか） | 文字だけの壁になる |
| 3 | **施設名・サービス名が並ぶ表は公式へリンク** | `check-article-ux.py` が落とす |
| 4 | **画像は必ず出典を書く**（`figcaption > cite` / `source-note` / `event-picks-credit`） | ライセンス違反 |
| 5 | **公開前に `npm run build` → `check-article-ux.py` を通す** | 崩れたまま出る |
| 6 | **使う画像は必ず自分の目で見る**（`Read` で開く） | 題材違い・不適切な画像がそのまま表紙になる |
| 7 | **対象が複数なら 1 対象 1 セクション**（表だけで済ませない） | 「一覧はあるが、個々が分からない」記事になる |
| 8 | **X の実投稿と YouTube を埋め込む**（実在するものだけ） | 一次の声が無く、どこにでもある要約記事になる |
| 9 | **関連記事を本文の中に最低 2 箇所置く**（末尾の自動一覧は数に入れない） | 読み終わりでサイトを出ていく |
| 10 | **ブランド名の左にロゴを置く**（**すべての記事**・見出しも表のセルも・もれなく全て）。残りは `npm run logos:missing` で数を出す | 「ちゃんとそのサービスのロゴ使って」と差し戻される |

## 1. アイキャッチ（最優先・後回し禁止）

**`eyecatchUrl` を書かないと、カテゴリ共通のフォールバック画像が表紙になる**
（`src/layouts/PostLayout.astro`）。**ビルドは通るし警告も出ない。** だから気づけない。
記事のテーマと無関係の画像が表紙として公開される。

### フロントマターに必ず両方書く

```yaml
eyecatchUrl: "/images/<slug>/eyecatch.jpg"
eyecatchAlt: "<記事タイトルと同等の説明>"
```

### 作り方

1. **背景写真を Wikimedia Commons から取る。** ライセンス台帳とセットで。

   ```bash
   python3.11 scripts/fetch-commons-photo.py --search "Sauna Finland" --limit 8
   python3.11 scripts/fetch-commons-photo.py --file "File:Xxx.jpg" \
       --key hero --dir public/images/<slug>/photos
   ```

   `_manifest.json` に `lic` / `artist` / `page` が記録される。**手で拾わない。**

2. **写真ヒーロー型で 1600×900 を生成する。**

   ```bash
   python3.11 scripts/gen-photo-hero.py \
     public/images/<slug>/photos/hero.jpg \
     public/images/<slug>/eyecatch.jpg \
     "<キッカー>" "<タイトル1行目>" "<タイトル2行目>" "<サブ>" "<出典クレジット>"
   ```

   構図を変えたいときは既存の `scripts/gen-*-eyecatch.py` を複製して調整する
   （`gen-sauna-eyecatch.py` / `gen-outlet-eyecatch.py` などが実例）。

3. **できた画像を必ず開いて見る。**

   ```text
   Read(file_path="…/eyecatch.jpg")
   ```

   **これを飛ばさない。** 2026-08-22、`sauna openings` の記事で
   検索語 `sauna interior wood` の 1 件目を無条件に採用したところ、
   **19 世紀の木版画「蒸し風呂に入る女性患者たち」**が選ばれた。
   題材が合わないうえ裸体が写っており、表紙として使えるものではなかった。

   **検索語が正しくても、1 件目が使える画像とは限らない。**
   Commons には絵画・古い版画・図版が大量に入っている。

### 対象が複数の記事は「タイル合成」にする

**施設一覧・イベント一覧のように対象が複数ある記事は、1 枚の写真では中身が伝わらない。**

#### 標準形は「全面タイル ＋ 透過オーバーレイ」（2026-09-05 に指示された・原則これ）

**タイルで画面全体を埋め、その上に暗幕を透過で重ね、さらに上からロゴとタイトルを載せる。**

> 表紙の画像はこの記事みたいな画像全体が6枚の画像でタイル分けされていて、
> 画像自体を透過して、上からタイトルやロゴなどを乗せる形の方がいいので、以降は原則そうして

見本は `outlet-mall-guide-2026` の表紙。

```bash
python3 scripts/gen-mosaic-hero.py OUT.jpg KICKER T1 T2 SUB CREDIT img1 … img6
```

- **左上と左下には「静かな写真」を置く。** そこは暗幕が濃く、文字が乗る場所。
  賑やかな写真を置くと文字も写真も両方死ぬ。
  `gen-outlet-eyecatch.py` のコメントにも「左上は暗部に入るので賑やかすぎない gotemba」とある
- **いちばん目を引く写真は右上へ。** 暗幕が薄く、そのまま出る
- **見出しは帯の幅に自動で収まる**（2026-09-05 に対応）。それ以前は 104px 固定で、
  長い見出しが明るいタイルの上まで伸びて読めなくなっていた
- **6 枚そろわないなら合成しない。** 足りないまま作ると同じ画像が繰り返されて破綻する
- タイルは**対象そのものの写真**を使う。街の風景で代用すると「施設一覧」に見えない
- タイルは**別々の対象**にする。同じ施設の写真を 2 枚入れると「6 施設ぶん」に見えない

**左に不透明な帯を作る `gen-tile6-hero.py` は、原則もう使わない。**
タイルに札を出したい場合など、明確な理由があるときだけ。

#### 素材の集め方

- 施設写真は Commons に無いことが多い。**公式サイトのギャラリーから取る**
  （クレジットは「画像: 各施設公式サイト」）
- **`og:image` を当てにしない。** 2026-08-27 に 6 施設で試したところ、
  5 枚中 4 枚がロゴやサイト共通バナーだった。**`og:image` は SNS シェア用の
  ブランド画像であって施設写真ではない。** ページ内の `<img>` と CSS の `url()` から
  横長で 640×360 以上のものを拾うこと

作ったら必ず `Read` で開いて、**6 枚を数える。**

#### 表紙のサブタイトルに「事実の羅列」を置かない

2026-09-04 に、モーニング記事の表紙が **「終わりは10:00・10:30・11:00」** で
差し戻された。指摘は「**なにも記事の魅力を答えてない**」。

サブタイトルは本文の要約ではなく、**読む理由**を書く場所。数字を並べるだけなら
見出しと同じことをしているだけで、1 行ぶんの面積が無駄になる。

- 悪い: 「終わりは10:00・10:30・11:00」（見ただけでは得も損も分からない）
- よい: 「最安180円。行ける時間は店で1時間ちがう」（安さと、逃す危険が同時に立つ）

#### 表紙の隅の文字は、まず生成器のコードを読む

2026-08-27 に、表紙の右下に出た `@heng_ji31590` を**「拾ってきた写真に入っていた
第三者の透かし」だと判断して、その写真を記事から外した。** 誤りだった。
`scripts/gen-mosaic-hero.py` が**このサイト自身のハンドルとして刻印している**もので、
写真には何も入っていなかった。

```bash
grep -n 'heng_ji31590' scripts/gen-*.py   # 生成器が何を描いているか先に見る
```

- **合成後の画像だけを見て、素材の欠陥だと決めない。** 素材を等倍で開いて確かめる
- 素材を外す前に、**外す理由が素材の側にあることを確認する**

それでも**本物の透かしは弾く。** 公式サイトから拾った写真でも、中身が第三者の
ストック写真のことがある。切り抜いて消すのは出典表示を削る行為なので不可。
その施設の写真として使わない。

### バナーを避ける条件を作りすぎると、本命を捨てる

**「バナーを弾く」は、やりすぎると施設写真そのものを落とす。**

2026-08-30 のららぽーと記事で、公式の施設ページから写真を 63 枚取ったところ
**1 枚も施設写真が無かった。** 取れていたのは学校の集合写真、太陽光の相談会チラシ、
パイ菓子の商品写真、テナントのロゴ。

原因は自分で書いた除外条件。

```python
BAD = re.compile(r"logo|icon|...|kv|main_?visual|common|header|footer")  # ← やりすぎ
```

**施設ページのキービジュアルこそが施設写真**なのに、`kv` と `main_visual` で
それを落とし、残ったイベント一覧・ショップニュースの画像を拾っていた。

**ハズレの原因は、ファイル名ではなく「どの階層の画像か」だった。**

```python
BAD      = r"logo|icon|favicon|sprite|placeholder|noimage|arrow|btn|sns"  # ロゴ類だけ
BAD_PATH = r"/(event|shopnews|pickup|topics|openrenewal)/"                # 階層で弾く
```

**og:image は候補に入れてよい。** 「og:image は SNS 用のブランド画像」は
**施設一覧のタイル**を作るときの話で、**個別ページのヒーロー**を探すときは逆に本命に近い。

判断の順は次のとおり。

1. **まず数枚だけ取って目で見る。** 21 施設 × 3 枚を一気に取る前に、1 施設で確かめる
2. ハズレていたら、**弾き方を足すのではなく、外れている理由を先に特定する**
3. 公式で取れなければ**プレスリリース**、それでも無ければ **Wikimedia Commons**（下記）

### スーパーの公式トップページからは、写真が 1 枚も取れない（2026-09-06 に実測）

**商業施設と同じやり方は通じない。** ららぽーとは施設ページのキービジュアルが
施設写真だったが、**スーパーチェーンのトップページは丸ごと販促の場**で、
`img` を拾うと出てくるのはバナーだけ。

7 チェーンから 36 枚 取って、**1 枚も使えなかった。**

| 拾えたもの | 実物 |
| --- | --- |
| `ok-1.jpg`（1200x520） | **求人情報のバナー**（「関東・関西 求人情報」） |
| `maibasket-2.jpg`（1920x1080） | **トップバリュの「値上げしません」キャンペーンのバナー** |
| `trial-5.jpg`（1440x665） | **ブラーのかかった汎用ストック画像**。トライアルの店ですらない |
| `hanamasa-*`（290x143 / 718x88） | 小さなバナー |

**サイズがきれいに揃っていたら、それはバナー。** 1200x520 が 6 枚、
1080x418 が 3 枚——**同じ寸法が並ぶのは写真ではなく枠**である。
落とす前にサイズの一覧を見れば、開く前に気づける。

```bash
python3 -c "from PIL import Image;import glob;[print(f,Image.open(f).size) for f in sorted(glob.glob('*'))]"
```

**スーパーは Commons へ行く。** チェーン店の外観は撮影されて上がっている。
`scripts/fetch-commons-photo.py` がライセンス台帳ごと記録してくれる。

### 公式から取れないときは、プレスリリースを見る

**公式サイトに載っていない金額と写真が、その会社のプレスリリースには載っている。**
PR TIMES は**会社が自分で出した一次情報**なので、出典を書けば根拠として使える。

2026-08-30 のサウナ記事で、公式サイトを 2 周しても埋まらなかったものが、
**リリースを見に行った 1 周で埋まった。**

| 公式サイトでは | リリースでは |
| --- | --- |
| SHIAGARU は「60分／90分／120分」の区分だけ。**金額は予約カレンダーの中** | **60分1,600円〜／90分2,000円〜／120分2,300円〜**が但し書きごと出ていた |
| 荒木町Logout・黄金湯新宿・門仲SAUNAS LO は写真が拾えない | **どれも施設写真が4枚以上**載っていた |
| 門仲SAUNAS LO は「この夏オープン予定」のティザー図だけ | **実際のサウナ室の写真**があった |

```text
公式サイト → 取れなければ プレスリリース → それでも無ければ「取れなかった」と書く
```

**リリースの所在はクラウド側の検索で確定させてから渡す。**
`prtimes.jp/main/html/rd/p/<8桁>.<9桁>.html` の形。**番号を当て推量で組み立てない。**

画像は `prcdn.freetls.fastly.net/release_image/...` にあり、そのまま取得できる。
**それでも中身は目で見る。** 同じリリースの中に別店舗の写真が混ざっていることがある
（SHIAGARU の公式サイトでは、1 枚目が**福岡・天神店**の写真だった）。

### 候補は「見て選ぶ」

自動で 1 件目を採る運用はしない。`ops/tasks/022-sauna-photo-candidates.sh` が実例で、
**候補を 480px のサムネイルに落としてブランチへ push し、目で見てから決める。**

```text
検索 → 上位数件をサムネイル化して push → Read で見て選ぶ → 本採用して生成 → また見る
```

写真に求める条件。

- **現代の実写。** 版画・絵画・図版は使わない
- **裸体・人物の顔が主題になっていない**
- 横長で使える（カード背景・1600×900 に切っても破綻しない）
- ライセンスが再利用可（`fetch-commons-photo.py` が非フリーを弾くが、`_manifest.json` で確認する）

### 合成はクラウドセッションでもできる。取れないのは「素材」だけ

**「日本語フォントが無い」は誤り。** 2026-08-27 に `fc-list` を `noto|cjk` で絞って
探したせいで見落としていた。実際には IPA ゴシックが入っている。

```bash
ls /usr/share/fonts/opentype/ipafont-gothic/ipag.ttf   # 日本語フォント
pip install Pillow                                     # pypi は NO_PROXY に入っている
```

**だから合成・確認・作り直しはこのセッションで完結する。** 何度でも作り直して
`Read` で見ればよく、1 往復 30 分かかる `ops/tasks` を回す必要はない。

**Mac に投げるのは「外から取ってくる」ところだけ。** クラウドセッションは
egress が塞がれていて（`WebFetch` は全ホストで `EGRESS_BLOCKED`、`curl` は
CONNECT 403）、公式サイトの写真も Commons の画像も取れない。

```text
素材を取る    → ops/tasks で Mac（30 分以内に走る）
合成・確認     → このセッション（Pillow ＋ IPA ゴシック）
```

**画像が用意できるまで記事を公開しない。** 表紙なしで出すのは不可。

## 2. 記事の型

### フロントマター

```yaml
---
title: "<主題>｜<数字の入った具体的な引き>"        # 100字以内
description: "<50〜160字。数字を必ず入れる>"
publishDate: 2026-08-22
category: ["roundups", "comparisons"]              # config の enum のみ
tags: ["<主題>", "<固有名詞>", "2026年版"]
isPR: false
draft: false
eyecatchUrl: "/images/<slug>/eyecatch.jpg"
eyecatchAlt: "<説明>"
author: "hacker-ko"
references: ["<出典URL>", ...]
---
```

### 冒頭（この順を崩さない）

1. **読者の疑問をカギカッコで 2 つ** →「検索しても出てこない」と断じる
2. **「だからアタシが作った」** + 何を土台にしたか（公式資料名を出す）
3. `hakkako-says` で**一番の驚きを先に**言う（`expr-05-smug.png`）

```html
<div class="hakkako-says">
  <div class="hakkako-mascot"><img src="/images/expr-05-smug.png" alt="Daily Hackマスコット" /></div>
  <div class="hakkako-quote">先に一番の驚きを言っとく。<strong>…</strong></div>
</div>
```

### `## 30秒で分かる、〜` ＋ 写真カード 6 枚

**一覧・選択肢は必ず `.event-pick` の写真背景カード。** 文字だけの `.highlight-grid` を
4 枚以上並べるのは禁止（`check-article-ux.py` が落とす）。

```html
<div class="event-picks">
  <a class="event-pick" href="#<リンク先h2のid>" style="--pick-img:url('/images/<slug>/photos/xxx.jpg')">
    <span class="pick-date">総数</span>
    <h4>全国32施設</h4>
    <p>…<strong>数字</strong>…</p>
    <span class="pick-go">詳しく見る →</span>
  </a>
  …6枚…
</div>
<span class="event-picks-credit">カード画像は…より引用。</span>
```

**`href` の id は実在させる。** `.section-with-mascot` の中の h2 には自動で id が
振られないので、**その場合は `<h2 id="...">` を手で書く。**

### 本文セクション

- 見出しは `## <主題>｜<結論>` の形（例: `## 店舗別売上ランキング｜三井不動産の決算資料そのまま`）
- **検索語を見出しに入れる。** 内容が本文にあっても、見出しに語が無いと順位が付かない
  （`docs/analytics/lalaport-keyword-inventory.md` の実測）
- 節ごとに `.section-with-mascot` でマスコットを添える（表情は内容に合わせる）

```html
<div class="section-with-mascot">
  <div class="mascot-wrap"><img src="/images/expr-07-gasp.png" alt="Daily Hackマスコット" /></div>
  <h2 id="見出しのid">見出し</h2>
</div>
```

### 期限のあるものを下に置かない（2026-09-05 に差し戻された）

**調べた人が最初に確かめるものを、上に置く。**

> このキャンペーンの表示をもっと上に持っていってほしいのと、画像やXでの投稿も
> 含めて目立つようにしてほしい。調べた人がなにをチェックするか考えて記事を作ってみて

IKEA豊洲の記事で、**オープン記念キャンペーン（9/13まで）を記事の後半に置いていた。**
開店直後に「IKEA豊洲」を調べる人が最初に見たいのは、**いつまで何が安いか**。
基本情報やゾーンの解説より先に来る。

- **期限があるものは `## 30秒で分かる` の直後**に置く。日付を見出しに入れる
  （「オープン記念キャンペーン｜**9月13日までが本番**」）
- **表で終わらせない。** 実際に買った人の X 投稿と、値札が写った店内写真を並べる。
  「本当に下がっている」が伝わるのは、公式の告知文ではなく**実物**のほう
- 読者が迷う点は `checklist` で潰す（IKEA豊洲なら「クーポンは配布9月・利用10月でずれる」）

**記事の並び順は「読者が確かめる順」で決める。** 書き手が説明したい順ではない。

### 対象が複数あるなら、1 対象 1 セクション

**一覧表を出して終わりにしない。** 表は「全体を見渡す」ためのもので、
読者が本当に知りたいのは「自分が行く 1 つ」のこと。

対象ごとに小見出しを立て、次を書く。

| 書くこと | 補足 |
| --- | --- |
| 見出しに**対象名をそのまま入れる** | 「高輪SAUNAS」で検索した人がここに着地する |
| 料金・最寄駅・男女の別 | 表と同じ値。**節だけ読んでも完結させる** |
| その施設だけの特徴 | 「日本初のトラムサウナ」のように 1 つ具体を出す |
| 公式へのリンク | 必ず張る |

**対象が 15 個あるなら 15 節書く。** 多いから表でまとめる、は逆。
検索は対象名で来るので、節が無いとその流入を丸ごと落とす。

節の並びはこの順で固定する。**節ごとにばらつかせない。**

```text
### ① 施設名（開業日・都県）
写真（あれば。figcaption に出典）
仕様表（最寄・料金・男女の 2 列）
その施設だけの一文
X の実投稿 か YouTube（あれば）
```

**素材が無い節は素材無しで出す。** 埋めるために別の施設の写真を流用したり、
一般的なサウナ写真で代用したりしない。

### ブランド名の左にはロゴを置く（2026-09-20 に指示・例外なし）

> ロゴはちゃんとそのサービスのロゴ使って
> ブランド名の左隣にスーパーのロゴ載せて。今後もそれをスキル化したい。
> サービス名の左にロゴを表示して

**チェーン・サービス・ブランドを名前で並べたら、その左にロゴを置く。**
見出しでも、表のセルでも、順位のリストでも同じ。**「もれなく全て」。**
**新しく書く記事は最初からそうする。既存の記事にも入れていく**（2026-09-20 に指示）。

2026-09-04 にモーニング記事で同じ指摘を受け、2026-09-20 にスーパー記事でまた受けた。
**一度やった記事にしか入っていないのが原因**なので、ここに書いて既定にする。

#### 置き場と名前

```text
public/images/<slug>/logos/<brand-key>.png
public/images/<slug>/logos/_manifest.json   ← **取得元を必ず残す**
```

#### 書き方（CSS は `global.css` にもう在る。増やさない）

見出しに置くとき。`<h3>` に `brand-h`、`<img>` に `brand-logo`。
**タイトルは `<span>` で包む**（包まないと折り返しが崩れる）。

```html
<h3 class="brand-h" id="-オーケー"><img class="brand-logo"
  src="/images/<slug>/logos/ok.png" alt="オーケーのロゴ" loading="lazy" /><span>①
  オーケー｜経費率17.4%で「毎日安い」を成立させている</span></h3>
```

表のセルに置くとき。`<td>` の中を `cell-brand` で包み、`brand-logo-sm` を使う。

```html
<td><span class="cell-brand"><img class="brand-logo-sm"
  src="/images/<slug>/logos/ok.png" alt="" loading="lazy" /><a
  href="https://ok-corporation.jp/" target="_blank" rel="noopener">オーケー</a></span></td>
```

- **ロゴは改行して、名前の上に置く**（2026-09-20 に指示）。
  **`cell-brand` は `display: block` で、`block` にするのはロゴだけ。**
  flex の縦積みにすると**セル内のリンクまで 1 行ずつ折れて列が潰れる**（同日に差し戻された）
- **高さだけそろえる。** 横長と正方形が混ざるので `width` は決め打ちしない
- **透過 PNG は白地の枠に載せる。** そのまま置くと背景に沈む（CSS 側で対応済み）
- **`alt` は表の中では空文字**にする。隣にブランド名のテキストがあるので二重に読まれる

#### 勢力図・散布図の点にも置く（2026-09-20 に差し戻された）

> 店舗のロゴ画像にしろって言ったよね？ちゃんと修正して。指示を聞いて。

**表と見出しに入れただけで「やった」と思ったのが間違い。** 図の中の札
（`.pos-dot b`）も名前が並んでいる場所なので、そこにも置く。
`b.with-logo` ＋ `img.brand-logo-xs` を使う。

**ただし札が重なっても、点の座標は動かさない。** 座標は実データなので、
ずらすと図が嘘になる。**向き（`lbl-left` / `lbl-right`）と `.pos-map` の高さで解く。**

#### 取り方｜「取れない」と言う前に、名前で調べる（2026-09-20 に叱られた）

**`ops/tasks` で取る。** クラウドからは大半のサイトに出られない。
`t115-logos-by-name.sh` を写す（`t111` / `065` は**探し方が浅い**版）。

**トップページの `img` を漁って無ければ諦める、をやって差し戻された。**

> 取れないわけないだろ。ちゃんとロゴの名前で調べて取得してきて。

**実際には在った。** 当たる順はこれ。

| # | 当たる先 | 実際に在ったもの |
| --- | --- | --- |
| 1 | **公式のメディアキット・広報ページ** | トライアルは**ロゴ ZIP を配布**していた（`trial-holdings.inc/news/media/`） |
| 2 | プレスリリース | PR TIMES の企業ページ。利用者から URL を渡されたこともある |
| 3 | **ウィキペディア／コモンズのファイルページ** | MediaWiki API で**実体 URL とライセンス表記**が同時に取れる |
| 4 | `og:image` / `apple-touch-icon` / class・alt に `logo` | **最後の手段。これだけで諦めない** |

**取れなかったときは、なぜ取れないのかを出す。** ページの `img` / `meta` / `link` /
svg 参照を丸ごとダンプして、**次を推測で決めない**。`t115` にその診断が入っている。

#### 選ぶときに外さないこと

- **運営会社のロゴを、店舗ブランドのロゴとして出さない。**
  神戸物産のロゴは業務スーパーのロゴではない。**会社名が書いてある行にだけ置く**
- **`og:image` は看板の写真やキャンペーンバナーであることが多い。** 必ず目で見る
- **縦長すぎるものは高さ 44px で潰れる。** 横長の lockup があるならそちらを選ぶ
- **余白は落としてよい**（トリミングではなく透明・単色の余白だけ）

#### 入れる前に、必ず全部を並べて目で見る（2026-09-20 に 3 回 助かった）

**`og:image` は「そのページを表す画像」であって、ロゴではない。**
実際に掴んだものを並べる。**見ずに入れていたら、全部そのまま公開されていた。**

| 入れようとしたブランド | 実際に写っていたもの | なぜ混ざったか |
| --- | --- | --- |
| Microsoft Azure | **Nestlé のロゴ** | 取引先カルーセルの og:image |
| Lucidchart | **HYUNDAI のロゴ** | 同上 |
| THE OUTLETS HIROSHIMA | **ACE Bags & Luggage** | テナントのロゴ |
| 毎日サウナ東京 | **スーパー銭湯**（ポータル） | 施設ページがポータル上にある |
| 水宴 | **フロサウナ**（ポータル） | 同上 |
| アルコイン | **Agoop**（運営会社） | 公式サイトが運営会社のもの |

**コンタクトシートにして 1 枚で見る。** 1 件ずつ開くと、見たつもりで飛ばす。

```python
# 取ってきた PNG を並べて 1 枚にする。白地で合成しないと透過が黒くつぶれる
im = Image.open(f).convert('RGBA')
bg = Image.new('RGBA', im.size, (255,255,255,255)); bg.alpha_composite(im)
```

**弾く基準はこの 4 つ。**

- **別のブランドが写っている** … 取引先・テナント・ポータル・運営会社
- **宣伝画像・製品のスクリーンショット** … Notion・Spotify・Figma・GitHub でこうなった
- **建物や乗り物の写真** … 施設系の og:image はほぼこれ
- **ロゴが小さく写り込んでいるだけ** … WILLER のバス写真がこれ

#### アプリのロゴは App Store のアイコンから取る

**アプリの公式サイトの og:image は、ほぼストアの宣伝バナー。**
提供元が登録した正方形アイコンを使うほうが確実で、`iTunes Lookup API` で取れる。

```text
https://itunes.apple.com/jp/lookup?id=<APP_ID>
  → results[0].artworkUrl512   ← **これが提供元の登録アイコン**
  → results[0].trackName       ← **必ず出す。** ID 取り違えにここで気づける
```

**`trackName` を出さないと ID の取り違えに気づけない。**
実際に d ヘルスケアの ID を間違えて**楽天ヘルスケアのページを見ていた**ことがある。

**検索の 1 件目をそのまま採らない**（2026-09-26 に 25 件 中 3 件 外した）。
`search` は**親ブランドと季節版を平気で上に返す。**

| 検索語 | 1 件目に返ったもの | 正しいもの |
| --- | --- | --- |
| `JAL Wellness Travel` | **JAL**（Japan Airlines） | id=1498726068 JAL Wellness & Travel |
| `楽天ポイントカード` | **楽天ペイ** | id=641501350 楽天ポイントクラブ |
| `Powl アンケート` | **ハロウィンの季節版** | 通常版に戻るまで採らない |

**`sellerName` が運営会社と合っているかを必ず出す。** 上位 3 件を並べて、
**コンタクトシートで既存と突き合わせてから**採る。正しい id が分かっていれば
`search` ではなく **`lookup?id=` で引く**（取り違えようがない）。

#### SVG は Mac で変換できない。持ち帰って Chromium でラスタライズする

**いまどきの公式サイトのロゴは、ほとんど SVG。** そして **Pillow は SVG を開けない**
（`UnidentifiedImageError`）。`ops/tasks` 側で変換しようとしても、
**Mac には `rsvg-convert` も `inkscape` も `magick` も `cairosvg` も入っていない**
（[`docs/mac-environment.md`](../../../docs/mac-environment.md) に実測がある）。

```python
# ops/tasks 側 … 中身が <svg> なら、変換せずそのまま置く
if blob[:400].lstrip()[:4] == b"<svg":
    open(f"{DIR}/{name}.svg", "wb").write(blob)
```

```bash
# クラウド側 … Chromium で PNG にする（透過のまま・2 倍 解像度）
node scripts/svg-to-png.mjs path/to/*.svg
```

#### 白抜きのロゴは、画像ではなく枠を暗くする

**公式が白版しか置いていないことがある**（AYANA・à la carte・Amiana で踏んだ）。
白地に置くと**何も無いように見える。** コンタクトシートでは「空の白い箱」になる。

**画像は加工しない。** 色を反転させるのは改変にあたる。**`class="... on-dark"` を足して、
枠の背景だけ暗くする。** CSS は `global.css` にもう在る。

#### 認証マークやテナントの札を掴まない

公式サイトの `logo` を含む img には、**ロゴではないもの**が普通に混ざる。

| 実際に掴んだもの | 何だったか |
| --- | --- |
| Travelife Gold Certified | **宿の認証マーク** |
| STGs STAR / GMP Quality | 同上 |
| ĐÃ THÔNG BÁO BỘ CÔNG THƯƠNG | **ベトナム商工省への届出バッジ** |
| プライバシーマーク | 同上 |
| Shibuya PIT ZERO / Sendai PIT | **同じ運営の別会場** |

**同じ運営の別会場・別ブランドが一番 危ない。** ファイル名では見分けがつかない
（`logo_pitzero.jpg` が Shibuya PIT ZERO だった）。**目で見るしかない。**

#### 入れたあと、スマホ幅で全記事を掃き出す

**目で見るだけでは見つからないものがある。** 実際に 2 つ見つけた（2026-09-20）。

```bash
npm run build && npx http-server dist -p 4321 -s &
node scripts/check-logos.mjs      # **390px 幅で全記事を開く**
```

| 見つかったもの | 原因 |
| --- | --- |
| **`h4` のロゴだけ高さ 89px**（他は 44px） | CSS が `h2.brand-h` / `h3.brand-h` と**レベルを決め打ち**していた |
| **512px のアイコンが素の大きさで出て、行が 300px 高くなった** | `.cell-brand` で囲み忘れたセル |

**横はみ出しはスマホ幅でしか出ない。** PC 幅で見て「大丈夫」と言わない。

#### どこが残っているかは、目ではなく数で出す（2026-09-26 に仕組みにした）

**「もれなく全て」は目では守れない。** 一度やった記事にしか入らず、
2026-09-04 と 2026-09-20 に同じ指摘を 2 回 受けた。**道具が 3 つ在る。**

```bash
npm run logos:missing                                          # ① どこが残っているか
python3 scripts/add-cell-logos.py <slug> --borrow --apply      # ② 表のセルを埋める
python3 scripts/add-heading-logos.py <slug> --borrow --apply   # ③ 見出しを埋める
```

| 道具 | 何を見るか |
| --- | --- |
| `find-missing-logos.mjs` | **入れていない箇所を探す**（Markdown を読む） |
| `check-logos.mjs` | **入れたロゴが壊れていないか**（ビルド後のページを開く） |

**②③ は別物。** `add-cell-logos.py` は表のセルしか見ないので、
**見出しが残っていても「0 件」と出る。** 2026-09-26 にそれで取りこぼしかけた。

`--borrow` は他記事の `logos/` から複製し、`_manifest.json` に
**取得元をそのまま引き継ぐ**（`copied_from` も残す）。**出所なしの複製はしない。**

**ブランド名の一次情報は `_manifest.json` の `brand`。**
ファイル名だけでは「`au.png` が au なのか auでんき なのか」が言えない。

#### 見出しを HTML にすると id が消える（アンカーが黙って切れる）

`### 楽天でんき` には **rehype が自動で id を付ける**が、
`<h3 class="brand-h">…</h3>` と生の HTML で書くと **id が付かない。**
記事内の `#…` リンクが**エラーも出さずに**切れる。

```bash
npm run build                                    # ← **先に必要**
python3 scripts/add-heading-logos.py <slug>      # dist から id を読んで書き写す
```

**ビルドしていなければ id が分からないので、その見出しは飛ばす作りにしてある。**
生の HTML の見出し（`<h3>楽天モバイル</h3>`）はもともと id が付かないので、
書き写すものが無い。**入れたあと、全記事のアンカーを `dist` と突き合わせる。**

```bash
python3 - <<'EOF'
import re, pathlib
bad = []
for md in pathlib.Path('src/content/posts').glob('*.md'):
    d = pathlib.Path(f'dist/posts/{md.stem}/index.html')
    if not d.exists(): continue
    ids = set(re.findall(r'id="([^"]+)"', d.read_text(encoding='utf-8')))
    for a in set(re.findall(r'href="#([^"]+)"', md.read_text(encoding='utf-8'))):
        if a not in ids: bad.append(f'{md.stem} #{a}')
print('切れているアンカー:', len(bad), bad[:5])
EOF
```

#### 機械が入れたものを、必ず目で見る（2026-09-26 に 1 件 取り消した）

**道具は「主役かどうか」しか見ていない。** ブランドの親子関係は分からない。

| 実際に起きたこと | どう直したか |
| --- | --- |
| **ソフトバンク（親ブランド）のロゴが「ソフトバンク光」に入った** | 消して `_todo` に「自前のロゴを取るまで置かない」と書いた |
| `楽天トラベル（楽天ふるさと納税）` で**括弧の中を拾った** | 手で 楽天トラベル に直した |
| `SBI損保 / チューリッヒ` を **1 ブランドと見なした** | 半角スラッシュも並列として弾くようにした |
| `au` が **「SHIB`AU`RA」** に当たった | 短い英字は語境界を要求するようにした |

**入れない箇所**（道具側で弾いている。手で足すときも同じ）。

- **1 見出し・1 セルに 2 ブランド以上**（`dヘルスケア / 楽天ヘルスケア`）。
  片方だけ差すと誤解を招く
- **並列のセル**（`楽天・d・PayPay・現金`）
- **親ブランドしか在庫が無いもの**（`ソフトバンク光` / `JAL Pay`）

#### 取りに行く前に、リポジトリの中を見る

**取得済みなのに記事で使っていないロゴが 14 点 あった**（2026-09-20）。
そのうち**モッピーとハピタスは、取りに行って取れなかったもの**だった。

```bash
# 使っていないロゴを洗い出す
python3 scripts/logo-inventory.py | awk -F'\t' '$1=="済"'
ls public/images/*/logos/*.png        # ← **先にここを見る**
```

#### 商標の扱い

**ロゴは商標であって、自由ライセンスではない。**
「どのチェーンの節か」を示す識別目的で使う。以下を必ず守る。

- **改変しない。** 色を変える・切り抜く・角を丸める（CSS の枠は可）はしない
- **`_manifest.json` に取得元 URL と取得日を残す。** どこから取ったか言えない画像は使わない
- **記事の顔（アイキャッチ）にロゴだけを大きく置かない。** 発信元だと誤解される

### X の実投稿を埋め込む

**行った人の声が無い記事は、どこにでもある要約記事になる。**
対象ごとに、その施設に触れている実投稿を 1 つ埋め込む。

```html
<blockquote class="twitter-tweet"><p lang="ja" dir="ltr">（投稿本文）</p>
&mdash; 表示名 (@handle) <a href="https://twitter.com/handle/status/ID">日付</a></blockquote>
<script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>
```

`blockquote.twitter-tweet` は `global.css` に定義済みで、`widgets.js` が
読み込めなくても**カードとして読める形で出る。**

- **実在する投稿だけ。** URL を組み立てて作らない。**捏造は記事全体の信用を壊す**
- **本文・投稿者・日付は syndication API で取る。** 検索結果の抜粋を信じない

  ```bash
  curl -s "https://cdn.syndication.twimg.com/tweet-result?id=<TWEET_ID>&lang=ja&token=a"
  # → text / user.name / user.screen_name / created_at
  ```

- クラウドからは x.com に到達できないので、`ops/tasks` 経由で Mac に探させる
- 宣伝ではなく**体験の投稿**を選ぶ。公式の告知だけ並べても読者の役に立たない

### YouTube の Vlog を埋め込む

サウナ・施設・イベントの記事は、**映像があると滞在時間が伸びる。**
その対象を扱った動画があれば埋め込む。

```html
<div class="yt-embed"><iframe src="https://www.youtube-nocookie.com/embed/<VIDEO_ID>"
  title="（動画タイトル）" loading="lazy" allowfullscreen
  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"></iframe></div>
```

`.yt-embed` は `global.css` に定義済み（16:9・角丸・最大 720px）。**インラインの
`style` を書かない。**

- **実在する動画 ID だけ。** 存在しない ID は再生できず、崩れて気づかれる
- **タイトルは oEmbed で照合する。** スクレイプしたタイトルは文字化けすることがあり、
  それを信じると**別の施設の動画を貼る。** 2026-08-27 に 3 本がこれで弾かれた
  （`suien` → 麻布十番の別店、`kaizoku` → **山賊**サウナ、`monnaka` → 幕張の別店）

  ```bash
  curl -s "https://www.youtube.com/oembed?format=json&url=https://www.youtube.com/watch?v=<ID>"
  # → title と author_name が返る。対象施設と一致しなければ使わない
  ```

- `youtube-nocookie.com` を使う

### 読者の「次の疑問」を先回りする

**記事の中で強い事実を出したら、そこから派生する検索を必ず拾う。**

| 記事が出した事実 | 読者が次に検索すること | 立てるべき節 |
| --- | --- | --- |
| 男性専用が半分以上 | 「サウナ 女性 入れる」 | **女性が利用できる施設だけの節** |
| 料金に 6.7 倍の開き | 「サウナ 安い」 | 予算別の節 |
| 7 月に集中 | 「サウナ 新店 いつ」 | これから開く施設の節 |

**言いっぱなしにしない。** 「男性専用が多い」で終わると、女性の読者はそこで離脱する。

### 使う部品

| 用途 | マークアップ |
| --- | --- |
| 比較表 | `<div class="cmp-table-wrap"><table class="cmp-table">`。推す行は `<tr class="recommended">` |
| 1 対象の仕様（最寄・料金など） | `<table class="cmp-table spec-table">` に `<tbody>` だけ。ラベルは `<th>` |
| 写真 | `<figure class="rn-figure">` + `figcaption` に `<cite>出典: <a …></cite>` |
| 手順・分類の解説 | `<ul class="checklist"><li><div class="checklist-body"><strong>①…</strong><p>…</p>` |
| 年表・時系列 | `<div class="tower-timeline"><div class="tl-year"><p class="tl-label">…<ul class="tl-items">` |
| 少数の紹介（3件程度） | `<div class="highlight-item"><span class="highlight-tag">…</span><h4>…</h4><p>…</p>` |
| 出典 | `<p class="source-note">出典：<a …>…</a>／…</p>` |
| 関連記事 | `<aside class="related-block">`（`related-block-thumb` に画像を入れる） |

**マークアップは記憶で書かず、上の 3 本から実物をコピーして中身を差し替える。**

#### 生の HTML ブロックの中で `**` は効かない

Markdown の強調は**HTML ブロックの内側では処理されない。**`<table>` や `<div>` の
中に `**〜**` を書くと、そのまま `**` として表示される。`check-md-bold.py` が
拾うが、**HTML を書いたら最初から `<strong>` を使う。**

`**〜**` が効かないもう 1 つの型が、**閉じ側の直前に句読点や引用符が来る**場合。

```text
✗ **"まちのリビング"**がコンセプト   → 「**まちのリビング**」がコンセプト
✗ **無料。**18:30〜                 → **無料**。18:30〜
```

### 締め

- **推測値を載せない。** 埋められなかったところを埋めたふりで書かない。
  ただし **「取れなかった」と記事に書くのも禁止**（§3 の「制作の裏側を記事に書かない」）。
  出せないなら、その項目ごと書かない
- 関連記事ブロック
- `hakkako-says` で締める（`expr-01-wave.png`）

### 関連記事は本文の中に最低 2 箇所（2026-09-05 に指示された・例外なし）

> おすすめ記事・関連記事をもっと文章内に入れるようにして。最低2箇所以上にれるようにして

**末尾の「同じカテゴリの記事」は数に入れない。** 読者が読み終わる前に、
本文の h2 の切れ目で 2 回以上ぶつかる状態にする。

- 置き方は `<aside class="related-block">`（§2「使う部品」）。**手書きが最優先**
- **`PostLayout.astro` が不足分を自動で補う。** 手書きが 0 箇所なら 2 個、1 箇所なら 1 個を
  h2 の切れ目へ差し込む（h2 が 8 本以上ある長い記事はもう 1 個）。
  候補は「共有タグ数 > 同じピラー > 同じカテゴリ」で選ぶ
- **本文中に既に出てくる `/posts/…` へのリンクは自動側から外れる。** 同じ記事へ 2 回
  誘導しない。手書きで置いたものと重複しないので、手書きを消す必要はない
- **文脈の合う記事を手で置くほうが強い。** 自動は保険であって、
  「この段落を読んだ人が次に読むもの」を選べるのは書き手だけ

### 広告枠は記事側では触らない

**記事の Markdown に広告タグを書かない。** 枠は `PostLayout.astro` が持っていて、
1 記事あたり最大 5 箇所（冒頭・本文中 2・末尾・サイドバー）＋スマホ下部固定に出る。

- 本文中の 2 枠は **h2 の 1/3・2/3 の位置**へ JS が移動する。h2 が 6 本未満なら 1 枠、
  2 本未満なら 0 枠に減る。**短い記事に広告を詰め込まない**
- 枠の実体は忍者AdMAX のタグ（`src/data/admax-tags.ts`）。
  **タグが未登録の slot は何も描画されない**（`PLACEHOLDER` で始まる値）。
  枠を増やしても、管理画面でタグを作って貼るまでは表示は増えない
- **記事を書くときに気にするのは 1 点だけ。** h2 が 2〜3 本しかない薄い記事にしない

**枠の一覧と有効化の手順は [`docs/ad-slots.md`](../../../docs/ad-slots.md) にある。**

## 2-B. 題材を選ぶ基準｜「数字の賞味期限」を見る

**書く前に、その題材の数字が何か月もつかを考える。** 2026-08-30 に
「回転寿司チェーンの店舗数ランキング」を提案して、利用者から差し戻された。

> 回転寿司は比較しづらいし値段がしょっちゅう変わるからどうだろうか

**両方とも正しく、実測とも一致していた。**

| 数字の種類 | 賞味期限 | このブログでの実測 |
| --- | --- | --- |
| 面積・店舗数・開業年・決算の売上 | **年単位** | ららぽーと記事＝表示413（サイト全体の67%） |
| 価格・キャンペーン・時限の割引 | **月単位** | クレカ／銀行キャンペーン系＝**表示1〜3で沈んでいる** |

`credit-card-campaign-2026-07` / `june-2026-campaigns-roundup` /
`may-2026-bank-campaigns-roundup` は、いずれも短命な数字の記事で、
**45 記事のなかで軒並み伸びていない。**

### 比較の軸が何本あるかも見る

**軸が 1 本しかない題材は、比較記事として弱い。**

| | 軸 | 判定 |
| --- | --- | --- |
| ららぽーと | 面積／店舗数／売上（**定義が揺れない 3 本**） | 成立した |
| 回転寿司 | 「1皿◯円」だけ。しかも皿の中身が各社で違う | **成立しない** |

ネタもサイズも違うものを「1皿の値段」で並べても、**公平な比較にならない。**

### 短命な数字を扱うときの逃げ方

価格が主題の記事を書くなら、**軸を「いくら」から動かす。**

- ✅ **「何時まで」「何がついてくるか」**（提供時間・セット内容は価格より安定する）
- ✅ 仕組みそのもの（コメダの「ドリンク代のみでトースト＋ゆで卵」は長年変わっていない）
- ✗ 価格の細部を主題にする

**価格は表に入れてよいが、記事の主題にはしない。** そのうえで
「料金は改定される。最新は公式で確認すること」を必ず書く。

## 3. 中身の作法

- **数字で答える。** 「多い」ではなく「20 施設中 12＝6 割」。他所が出していない
  切り口の数字を 1 つは作る（順位づけ・比率・倍率）
- **数値が割れたら、割れた理由を書く。** どちらが誤りとは書かない。
  ららぽーと記事の「他サイトと面積・店舗数が食い違う理由」が実例
- **出典は一次情報。** 決算資料・プレスリリース・公式サイト。まとめサイトを根拠にしない
- **一人称は「アタシ」**、マスコットは**ハッカー子**
- 参考にした外部記事・投稿の**本文は流用しない。** 題材を起点に自分で裏を取り直す

### 制作の裏側を記事に書かない（2026-09-05 に差し戻された・例外なし）

**「取れなかった」「載せていない」「除外した」は、読者に向けた文ではない。**
それは書き手の作業ログであって、読む人には何の価値も無い。

2026-09-05、モーニング記事が次の指摘で差し戻された。

> この話は記事で書く必要ないのでやめて
> こういう内部向けの内容を記事で書かないで
> 以降も記載しないでほしいので、スキルに明確に記録しておいてほしい

**記事に書かないもの**（見出し・表・キャプション・冒頭、どこであっても）。

| 書かない | なぜ |
| --- | --- |
| 「公式ページから金額が取れなかった」 | 取得作業の報告。読者は取得していない |
| 「この記事に載せていない／入れていない」 | 編集方針の説明。読者は載っているものだけ見る |
| 「品名までしか確認できなかった2社」のような見出し | 見出しが**弱点の告白**になっている |
| 「この記事で『分からない』と書いたこと」の節 | 節まるごと内部向け |
| 「無関係な画像を貼るくらいなら無いと書く」 | 判断の弁明。読者には関係ない |
| 「まとめサイトは根拠にしていない」 | 手続きの自慢 |

**代わりにどうするか。**

- **書けることだけ書く。** 金額が無いなら、種類・特徴・つくものを書く。
  情報が薄い対象でも、**節の見出しは他と同じ形にそろえる**
  （「品名までしか確認できなかった2社」→「⑨ ドトールコーヒー『モーニング・セット』」）
- **注意書きは読者の言葉に直す。** 「公式から取れなかった」ではなく
  「店舗によって異なる」「価格は改定されることがある」——**読者の行動が変わる形**にする
- **素材が無い対象を無理に埋めない。** ただし**埋められない理由を記事に書かない。**
  黙って、書けることだけを書く

**「誠実さ」の見せ方を間違えないこと。** 裏取りの厳しさは**中身の正確さ**で示すもので、
できなかったことの列挙で示すものではない。

## 4. 公開前の検証（全部やる）

```bash
npm run build                                   # 通ること
npx astro check --minimumSeverity error         # **CI はこれも見る。** build だけでは通る
python3 scripts/check-article-ux.py dist/posts/<slug>/index.html   # 指摘ゼロにする
python3 scripts/check-md-bold.py dist/posts/<slug>/index.html      # ** の崩れ
```

#### `npm run build` が通っても `astro check` は落ちる

**別物である。** `build` は型を見ない。CI（`test-build.yml`）は
`npx astro check --minimumSeverity error` を別ステップで走らせるので、
**ローカルで build だけ見て push すると CI で落ちる。**

2026-09-20 に踏んだ。`post.body` は **`string | undefined`** で、
そのまま関数へ渡して 3 件 の型エラーになった。**build は 517 ページ 成功していた。**

#### ビルドの成否を先に見る。落ちていても検査 2 本は通る

**検査スクリプトは `dist` を読む。** ビルドが落ちると `dist` は前回のまま残るので、
**古い HTML を見て「OK」と言ってくる。** 2026-08-28 に `description` が
160 字の上限を超えてビルドが落ちたのに、検査 2 本は通った。

```bash
npm run build 2>&1 | tail -2      # ← ここで "Complete!" を確認してから検査へ進む
```

`&&` でつないで、ビルドが落ちたら検査に進まないようにするのが安全。

#### フロントマターの上限

| 項目 | 上限 |
| --- | --- |
| `title` | 100 字 |
| `description` | **160 字**（超えるとビルドが落ちる） |

#### 生 HTML の見出しには必ず `id` を書く

**`.section-with-mascot` の中に `<h2>` を生 HTML で書くと、`id` が自動で振られない。**
`id` が無い見出しは、目次にもページ内リンクにも出せない。

```html
<h2 id="料金">…</h2>   ← id を手で書く。書き忘れても ビルドは通る
```

目次は `src/lib/collectHeadings()` が Markdown の `##` と 生 HTML の `<h2 id="…">` を
文書順にマージして作る。**`id` が無いものはマージ対象から落ちる。**

2026-08-30 に、**サウナ記事の H2 10 本が目次に 1 本も出ていなかった**のを見つけた。
目次が短いだけなので、ページを見ても異常だと分からない。

```bash
# 記事内の h2/h3 が全部 目次に出ているかを機械で確認する
python3 - <<'EOF'
import re
h=open('dist/posts/<slug>/index.html',encoding='utf-8').read()
toc=re.search(r'<nav class="toc".*?</nav>',h,re.S).group(0)
t={re.sub('<[^>]+>','',x).strip() for x in re.findall(r'<a [^>]*>(.*?)</a>',toc,re.S)}
art=re.search(r'<article.*?</article>',h,re.S).group(0)
print('目次に無い見出し:',[re.sub('<[^>]+>','',m.group(2)).strip()
  for m in re.finditer(r'<h([23])[^>]*>(.*?)</h\1>',art,re.S)
  if re.sub('<[^>]+>','',m.group(2)).strip() not in t])
EOF
```

#### 最初の見出しを生 HTML にすると markdownlint が落ちる

記事の最初の見出しを `.section-with-mascot` の `<h2>` にして、その次に `###` を書くと
**MD001（見出しは 1 段ずつ）でコミットが弾かれる。** markdownlint からは
生 HTML の `<h2>` が見えないため。**最初の H2 は Markdown の `##` で書く。**

#### ページ内リンクの id は、ビルド後 HTML から取る

`### ① PARADISE 大手町（2/1・東京）` から自動生成される id は
`-paradise-大手町21東京` で、**丸数字も記号も落ちる。** 推測で書くと必ず外す。

```bash
npm run build
python3 - <<'EOF'
import re
h=open('dist/posts/<slug>/index.html',encoding='utf-8').read()
ids=set(re.findall(r'id="([^"]+)"',h)); hrefs=set(re.findall(r'href="#([^"]+)"',h))
print('リンク切れ:',[x for x in hrefs if x not in ids])
EOF
```

**未解決ゼロを機械で確認する。** 目で追わない。
`♻️` のような絵文字は**異体字セレクタが id に残る**ので、なおさら手では書けない。

さらに手で確認する。

- **`eyecatchUrl` のファイルが実在するか**（`ls public/images/<slug>/eyecatch.jpg`）
- **アイキャッチと記事内画像を `Read` で開いて、題材が記事と合っているか**
- **埋め込んだ X 投稿と YouTube が実在するか**（URL / 動画 ID を目で追う。組み立てた URL は必ず壊れる）
- **対象ごとの節がそろっているか**（15 施設なら 15 節。表だけで済ませていないか）
- 生成 HTML のタグ開閉が一致しているか
- **本文の数字を表から機械的に再集計して一致するか**（手で数えない）
- `.event-pick` の `href` の id がビルド後 HTML に存在するか

**どちらもビルド後の HTML を渡す。** `.md` を渡すと `check-md-bold.py` は
ソース中の `**` をすべて拾って大量に誤検出する（2026-08-22 に踏んだ）。

`check-article-ux.py` の指摘を「既存記事も出ているから」で見逃さない。
**新しく書いた記事は指摘ゼロで出す。**

## 4-B. 公開前に実際のページを描画して見る

**ビルドが通ってもページが正しいとは限らない。** 検査スクリプトは HTML の構造しか見ない。
**アイキャッチの文字が本文と食い違っていても、誰も止めてくれない。**

このリポジトリのクラウドセッションでもレンダリングできる。日本語フォント（IPA ゴシック）は
入っており、Chromium も置いてある。

```bash
npx --yes http-server dist -p 4321 --silent &
/opt/pw-browsers/chromium-1194/chrome-linux/chrome --headless --disable-gpu --no-sandbox \
  --hide-scrollbars --window-size=1200,3000 --virtual-time-budget=9000 \
  --screenshot=/tmp/shot.png "http://127.0.0.1:4321/posts/<slug>/"
```

撮ったら **`Read` で開いて見る。** 見るのは次の 4 点。

| 見るところ | ありがちな崩れ |
| --- | --- |
| **アイキャッチの文字と記事タイトルが一致しているか** | 対象を変えたのに画像を作り直していない |
| `.event-pick` の枚数と並び | 3 列グリッドなので **3 の倍数**にする。4 枚だと 3+1 で間延びする |
| 表の折り返し・はみ出し | 列が多い表はスマホ幅で崩れる |
| マスコットと吹き出しの位置 | `hakkako-says` の入れ子を間違えると崩れる |

**2026-08-22、対象を「主要20施設」から「首都圏15施設」に絞ったのに、
アイキャッチは「主要20施設」のまま公開した。** ページを描画して初めて気づいた。
検査 2 本とも通っていた。

## 5. 公開とその後

1. PR を作ってマージ（`main` は保護ブランチ・直 push 不可）
2. **プレビュー URL を報告に載せる**（下記・`CLAUDE.md` 最上位ルール 16）
3. 公開 URL が確定したら **X 告知は tweet2 に依頼する**
   （`docs/cross-session-requests.md` に記事 URL・要点 3 つ・画像パスを書く）。
   **blog3 は X に投稿しない**（`CLAUDE.md` 最上位ルール 4・5）
4. `npm run handoff -- "…" --next "…"` で引き継ぎを残す

### プレビュー URL を必ず一緒に出す（2026-09-13 に指示・例外なし）

> 記事はまず作成して公開してしまっていいのだけど、必ず開発環境のプレビューを
> 一緒に共有してレビューをできるようにして。

**公開の可否を確認する必要はない。そのまま出してよい。**
**だが、報告にプレビュー URL が無いとレビューができない。**

利用者は**下書きフィードバックの仕組みでページに直接 指摘を書き込む。**
これが使えるのは `*.pages.dev` のプレビューのほうで、**本番 URL では指摘が入れられない。**
2026-09-12 に本番 URL だけを出して「**いやプレビューのページでちょうだいよ／
フィードバックできないよ**」と差し戻された。

```text
PR を作る → GitHub Actions が Cloudflare Pages へデプロイ
          → **PR に bot がコメントで URL を書く**
          → その URL を報告に載せる → マージ
```

```text
mcp__github__pull_request_read(method="get_comments", pullNumber=<PR>)
  → "🚀 Preview deployment: https://<8桁hex>.daily-hack.pages.dev"
```

| | |
| --- | --- |
| **URL の形** | `https://<8桁hex>.daily-hack.pages.dev/posts/<slug>/` |
| **組み立てられない** | ハッシュはコミット由来。`<branch>.daily-hack.pages.dev` だと思って書くと外す |
| **マージ後も生きる** | 先にマージしてしまっても、PR のコメントから取れる |
| **ブランチ更新で変わる** | `update_pull_request_branch` のあとは**新しいコメントの URL**を出す |
| **パスまで付ける** | トップだけ渡すと相手に記事を探させることになる |

**記事を出したと報告する文に、プレビュー URL が入っていなければ未完了。**
