...
| 10 | **ブランド名の左にロゴを置く**（見出しも表のセルも。もれなく全て） | 「ちゃんとそのサービスのロゴ使って」と差し戻される |
...
`065-morning-logos-menus.sh` と `t111-super-logos.sh` を写す。

候補にするのは次の 3 つ。**この順で確度が高い。**

1. `og:image` … 1 枚に決め打ちされている
2. `apple-touch-icon` … 正方形で背景が付いていることが多い
3. `class` / `alt` / ファイル名に `logo` を含む `<img>`

**公式から取れないときはプレスリリースを見る**（この文書の §1 と同じ）。
利用者から「ロゴは例えばこれとかつかって」と PR TIMES の企業ページを渡されたこともある。
