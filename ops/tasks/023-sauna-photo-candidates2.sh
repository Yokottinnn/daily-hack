#!/bin/bash
# サウナ記事の写真候補 2 巡目。1 巡目で使えたのは 4 枚だけだった。
#
# 1 巡目（022）は sauna=サウナ用スツールの製作写真、rotenburo=海岸の崖、
# nagoya=券売機、sento の一部=浮世絵、と外れが多かった。
# 採用できたのは stove-3（実写のサウナ室）・sento-1・tokyo-2・yokohama-2 の 4 枚。
# ここでは足りないテーマ（ストーブ・水風呂・名古屋・福岡・大阪・休憩スペース）を取る。
...
BRANCH="claude/sauna-photo-candidates2"
...
{
  echo "# サウナ記事の写真候補 2巡目（$(date -u +%Y-%m-%dT%H:%M:%SZ)）"
  echo
  echo "**021 は検索 1 件目を無条件採用して失敗した。** ここでは候補を並べる。"
  echo "サムネイルはブランチ \`$BRANCH\` の \`candidates/\` にある。**見てから選ぶこと。**"
  echo
  for pair in \
    "sauna heater stones kiuas|kiuas" \
    "cold plunge pool spa|mizuburo" \
    "Nagoya skyline towers|nagoya" \
    "Fukuoka Hakata skyline city|fukuoka" \
    "Sakai Osaka city view|osaka" \
    "spa relaxation lounge chairs|lounge"; do
    q="${pair%%|*}"; tag="${pair##*|}"
    echo "## ${tag} — 検索語: \`${q}\`"
    echo
...
git -C "$WT" -c user.name="ops-heartbeat" -c user.email="noreply@fieldbeside.com" \
  commit -q -m "wip: サウナ記事の写真候補 2巡目（サムネイル・レビュー用）
