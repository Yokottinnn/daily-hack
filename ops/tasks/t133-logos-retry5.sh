#!/bin/bash
# **t130 で取れなかったぶんを取り直す（t133）。**
#
# t130 は 15 ブランド中 9 件 しか使えなかった。目で見た内訳:
#   - **モッピー** … 「現金」のコイン画像・V POINT・R POINT（提携先のロゴ）
#   - **ハピタス / 一休.com** … 形が合わず 1 枚も保存されなかった
#   - **じゃらん** … ページの**スクリーンショット**
#   - **Yahoo!トラベル** … Facebook・X・LINE の**SNS アイコン**
#   - **dカード** … ページが開けない（URLError）
#   - **エポスカード** … 「EPOS Net」（会員サイトの名前であってカードのロゴではない）
#   - **PayPayカード** … カードの**券面画像**
#
# **入口を変える。** サービスの専用ドメイン・ヘルプ・企業情報のページを当たる。
# コモンズも見る。土台は t126 / t128 と同じ。
#
# **保険スクエアbang! は諦める。** トップも会社ページも
# **運営会社のウェブクルーのロゴしか置いていない**（t129 で確認）。
# 運営会社のロゴをサービスのロゴとして出さない（最上位ルール 17）。
#
# **判断はしない。** **秘密は出さない。** LLM 不使用・$0/回・$0/日・$0/月

set -uo pipefail
RDIR="${OPS_REPORT_DIR:-/tmp}"
OUT="$RDIR/t133-logos-retry5.md"
DIR="$RDIR/logos-retry5"
mkdir -p "$DIR"
...
SEARCH = [
    ("epos",    "エポスカード",   "Epos Card logo"),
    ("dcard",   "dカード",        "NTT Docomo logo"),
    ("jalan",   "じゃらん",       "Recruit Holdings logo"),
]
APPS = []   # **この回はアプリを取らない**
SITES = [
    ("moppy",      "モッピー",       "https://pc.moppy.jp/about/"),
    ("moppy2",     "モッピー（企業）", "https://ceres-inc.jp/"),
    ("hapitas",    "ハピタス",       "https://sp.hapitas.jp/"),
    ("ikyu",       "一休.com",       "https://www.ikyu.com/corporate/"),
    ("jalan2",     "じゃらん",       "https://www.jalan.net/theme/"),
    ("yahootra",   "Yahoo!トラベル", "https://travel.yahoo.co.jp/help/"),
    ("dcard2",     "dカード",        "https://dcard.docomo.ne.jp/st/"),
    ("epos2",      "エポスカード",   "https://www.eposcard.co.jp/company/"),
    ("paypaycard", "PayPayカード",   "https://www.paypay-card.co.jp/company/"),
]
...
man = {"_note": "識別目的で使うロゴ。**商標であり自由ライセンスではない。**"
                "取得日 " + dt.date.today().isoformat() + "（ops/tasks/t128-logos-retry5.sh）。"}
t0 = time.time()
