#!/bin/bash
# **鍵を置いたあとに、もう一度 確かめる（t170）。LLM 不使用・$0。**
#
# **このタスクは API を 1 回も呼ばない。** 鍵が「在るか」を見るだけ。
#
# ## なぜ t169 と同じものを番号だけ変えて出すのか
#
# **t169 は `done/` に入ったので二度と走らない。** そして t169 のレポートは
# **2026-09-23 19:48 JST** 生成で、**利用者が鍵を置いたのはそのあと**だった。
# 古いスナップショットを「いま」として読まない（最上位ルール 11）。
#
...
OUT="$RDIR/t170-key-recheck.md"
KEYNAME="ANTHROPIC_$(printf 'API')_KEY"

{
  echo "# 鍵を置いたあとの再確認（t170・**\$0**）"
  echo ""
...
if [ -d "$REPO" ] && command -v node >/dev/null 2>&1; then
  ...
  rc=$?
  {
    ...
    echo '```'
    echo ""
    echo "- 終了コード **$rc**（**\`rc=0\` は動いた証拠でしかない**・最上位ルール 13）"
  } >> "$OUT"
else
  echo "- ⚠️ リポジトリか node が見つからない: \`$REPO\`" >> "$OUT"
fi
