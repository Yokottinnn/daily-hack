#!/bin/bash
# **PAY ID パターンA を予約投稿する（x158 の出し直し）。費用 $0。**
#
# ## `x158` はどこで止まったか
#
#   FAIL 文面が 3 本ではない
#   rc=1
#
# **画像は `origin/main` から取り直したのに、文面は Mac の作業ツリーから読んでいた。**
# 作業ツリーは main とは限らないので、`payid-a` がまだ無い古い `posts.json` を読んだ。
#
# **ガードが手前で止めたので、キューも plist も触っていない。** 状態はきれい。
#
#   → **文面も `git show origin/main:...` で取る。**画像と同じ取り方に揃える
#   → 外したときは **`origin/main` のキー一覧を出す**（次はもう推測しない）
#
...
SCHED_JST="2026-09-27 12:00"   # 2026-09-26 にダイアログで決定（JST・分まで）
# ============================================================
...
node -e '
  ...
  // **文面も origin/main から取る。** Mac の作業ツリーは main とは限らない
  // （x158 はここを作業ツリーから読んで「文面が 3 本ではない」で止まった）
  const { execFileSync } = require("child_process");
  const rawPosts = execFileSync("git", ["-C", repo, "show", "origin/main:scripts/image-review/posts.json"], { encoding: "utf8", maxBuffer: 8 * 1024 * 1024 });
  const posts = JSON.parse(rawPosts);
  const t = posts["payid-a"];
  if (!Array.isArray(t) || t.length !== 3) {
    console.log("FAIL 文面が 3 本ではない。origin/main のキー: " + Object.keys(posts).join(", "));
    process.exit(1);
  }
