#!/bin/bash
# **同じ実行と過去を分けて数え直し、返信案を 10 件 出す。投稿しない。**
#
# ## t033 が 0 件だった理由（私の設計ミス）
#
#   候補 8 → 入口で見送り 6（PR/アフィリ・$0）→ 生成 2 → **通過 0**
#
# 弾いた理由が「末尾の絵文字 💡 が直近 20 件に 2 件」。
# **比較先の「直近 20 件」は穴埋め時代の返信そのもの。** あれは意図的に型が
# 固まっていた出力なので、そこを基準にすると 14 種の絵文字がほぼ全部
# 最初から「使用済み」になり、**新しい文が 1 つも通らない。**
#
# ## 直した点 — 2 つの窓を分ける
#
#   同じ実行の中     … 並んで人目に触れる。**厳しく**（1 件で打ち止め）
#   日をまたいだ過去 … 何日も離れている。**緩く**（絵文字と書き出しは 3 件、締めは 2 件）
#
# `checkRelevance` に `runReplies` を足し、`asuka-reply.cjs` が
# `OPS_EXTRA_RECENT`（その実行で作った文）をそちらへ渡す。
#
# 手元の検証: 3 件を 1 回の実行として流し、**2 件目（同じ実行で 💡 が 2 つ目）だけを弾いた。**
#
# ## 候補の供給が足りない
#
# 検知 1 周で取れるのは 10 件ほどで、**うち 6 割が PR/アフィリ。**
# 返信できる相手は 1 周あたり 2 件前後しかない。**検知を 3 周 まわして貯める。**
# 検知は Playwright の DOM 取得のみで **LLM を使わない＝ 何周しても $0。**
#
# ## やらないこと
#
# **投稿しない。enqueue しない。フォローしない。ジョブも戻さない。**
#
...
OUT="${OPS_REPORT_DIR:-/tmp}/preview-ten-v2.md"
DEST="$W/ops"
...
{
echo "# 返信案 10 件（やり直し）（$(date '+%Y-%m-%d %H:%M:%S') JST）"
echo
...
# **検知を 3 周 まわす。** 1 周で取れるのは 10 件ほどで、うち 6 割が PR/アフィリ。
# 返信できる相手は 1 周あたり 2 件前後しかない（2026-09-05 に実測）。
# **検知は LLM を使わないので、何周しても $0。**
echo '[]' > /tmp/.t034raw.json
...
let d="";process.stdin.on("data",c=>d+=c);process.stdin.on("end",()=>{
  ...
  let all=[]; try{ all=JSON.parse(fs.readFileSync("/tmp/.t034raw.json","utf8")); }catch(e){}
  const seen=new Set(all.map(x=>String((x&&(x.text||x.full_text||x.content))||"")));
  for(const x of arr){
    const t=String((x&&(x.text||x.full_text||x.content))||"");
    if(t&&!seen.has(t)){ seen.add(t); all.push(x); }
  }
  fs.writeFileSync("/tmp/.t034raw.json", JSON.stringify(all));
  console.log(all.length);
});' > /tmp/.t034n 2>/dev/null || true
  echo "- 検知 ${pass} 周目 → 累計 **$(cat /tmp/.t034n 2>/dev/null || echo 0) 件**"
  sleep 5
done
DN="$(cat /tmp/.t034n 2>/dev/null || echo 0)"

# **相手を弾くフィルタは、呼べたときだけ通す。** 呼び方を推測しない
if [ "${DN:-0}" != "0" ] && [ -f "$SCRIPTS/ng-filter-candidates.cjs" ]; then
  F="$("$NODE_BIN" "$SCRIPTS/ng-filter-candidates.cjs" < /tmp/.t034raw.json 2>/dev/null || true)"
  if printf '%s' "$F" | "$NODE_BIN" -e 'let d="";process.stdin.on("data",c=>d+=c);process.stdin.on("end",()=>{try{const a=JSON.parse(d);if(Array.isArray(a)){require("fs").writeFileSync("/tmp/.t034raw.json",JSON.stringify(a));console.log(a.length);process.exit(0)}}catch(e){};process.exit(1)});' > /tmp/.t034f 2>/dev/null; then
    echo "- \`ng-filter-candidates.cjs\` を通した: **$(cat /tmp/.t034f) 件**"
  else
...
try{ JSON.parse(fs.readFileSync("/tmp/.t034raw.json","utf8")).forEach(pull); }catch(e){}
if(!out.length){
  try{
...
fs.writeFileSync("/tmp/.t034c.json", JSON.stringify(out));
console.log(out.length);
' "$W/data/trend-cache.json" > /tmp/.t034m 2>/dev/null || echo 0 > /tmp/.t034m
N="$(cat /tmp/.t034m 2>/dev/null || echo 0)"
...
while [ "$i" -lt "$N" ]; do
  ...
  [ "$OKC" -ge "$WANT" ] && break
  TARGET="$("$NODE_BIN" -e 'process.stdout.write(JSON.parse(require("fs").readFileSync("/tmp/.t034c.json","utf8"))[Number(process.argv[1])]||"")' "$i")"
  i=$((i+1))
  [ -z "$TARGET" ] && continue
...
rm -f /tmp/.t034raw.json /tmp/.t034c.json /tmp/.t034n /tmp/.t034m /tmp/.t034f
