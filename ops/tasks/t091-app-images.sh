#!/bin/bash
# **最後の 2 件を、日本語の正式名で取り直す。**
#
# ## なぜ英語名で引っかからなかったか
#
# `EveryPoint` で検索すると 3D スキャンの別アプリが返る。
# **App Store 上の名前は「エブリポイント」**（株式会社ピクセラ・id6743367643）で、
# 英語表記では出てこなかった。**検索語をサービスの英語名で決め打ちしない。**
#
# 東京健康UPlus は東京都のサービス。アプリが無ければ**アプリとして数えない。**
#
# ## t088 がなぜ空振りしたか
#
...
L = ["# 歩いてポイ活 21 アプリの App Store 素材（t091）", "",
