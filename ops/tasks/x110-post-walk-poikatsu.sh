#!/bin/bash
# **歩いてポイ活の告知スレッド（[1/2] + [2/2]）を X へ出す。**
#
# 本文と画像 4 枚は **2026-09-20 にレビューページとチャットで実物を見たうえで承認済み**
# （「内容Okなので投稿して」。最上位ルール 4「画像確認なしの投稿は絶対に許可されない」を満たす）。
#
# **文面は利用者がレビューページで自分で書き換えたもの**をそのまま使う。
# ただし「年間300〜1,000マイル」は記事の表の見出しが「月いくら」だったため
# **「月300〜1,000マイル」**に直してある（そのまま出すと 12 倍 になる）。
#
...
#     （`auto-x-publisher.js` を 2 回 叩いても [2/2] は出ない）
#   * `queue-manager.js approve` は**存在しない**ので呼ばない
#
# ## 踏んだ穴は全部 塞いである
#
#   1. **重みを先に数える。** 280 を超えると投稿ボタンが有効にならず、
#      **エラーらしいエラーも出さずに落ちる**（2026-09-05 に 40 分 失った）
#   2. **画像は `origin/main` から `git show` で取り出す。**
#      ポーラーは作業ツリーを main に切り替えないので、ディスクの絵は古いことがある
#   3. **CDP の口は 18810。** `9222` ではない。**ポートが開いているだけでは健全ではない**ので
#      `cdp-health.js` で見る（ハングした Chrome も 200 を返す）
#   4. **マスクは秘密だけに絞る。** 無差別なマスクと `cut` はエラー本文ごと潰す
#   5. **出したあとキューに書き戻す。** `run-publish.sh` は書き戻さないので、
#      放っておくと次が「まだ出ていない」と誤認して二重投稿する
#
# ## 二重投稿をしない
#
...
OUT="${OPS_REPORT_DIR:-/tmp}/post-walk-poikatsu.md"
LOCK="$W/data/.x110-walk-post.lock"
NODE_BIN="$(command -v node 2>/dev/null || echo /usr/local/bin/node)"
IMGDIR="$W/data/x-walk-poikatsu"
SRCDIR="public/images/walk-poikatsu-2026/x"
...
URL="https://daily-hack.fieldbeside.com/posts/walk-poikatsu-2026/"
...
T1='「歩いてお金が貯まる」は複数のサービスを組み合わせることで立派なポイ活になるわよ。マイルが貯まる仕組みもある。

・毎日8,000歩で年365円（無料）
・JALは月550円で、月300〜1,000マイル

だからアタシが歩いて稼げる20アプリを徹底分析したわよ。'

T2='「歩いてポイ活」で調べると、どのサイトも同じアプリを同じ順で並べてるだけ。金額が比べられないのよ。
...
try{
  ...
  console.log(rows.filter(e=>{
    ...
    return t.includes("walk-poikatsu-2026")
...
{
echo "# 歩いてポイ活の告知スレッドを X へ出す（$(date '+%Y-%m-%d %H:%M') JST・費用 \$0）"
echo
echo "> 本文と画像 4 枚は **2026-09-20 にレビューページとチャットで実物を見たうえで承認済み**。"
echo "> **文面は利用者がレビューページで自分で書き換えたもの。**"
echo "> ただし「年間300〜1,000マイル」は記事の表の見出しが「月いくら」だったため"
echo "> **「月300〜1,000マイル」**に直してある（そのままだと 12 倍 になる）。"
...
IMGS=""; MISSING=0
echo '```'
for f in 1-summary.jpg 2-waon.jpg 3-web3.jpg 4-mile.jpg; do
...
echo "**ログアウト中に走らせると [1/2] だけ出て片肺になる。**"
echo
echo '```'
...
if grep -q 'キューを posted に書き戻した' "$OUT" 2>/dev/null; then
  echo "**歩いてポイ活の告知を出した。chain の 2 本を確認すること** / $(basename "$OUT")"
