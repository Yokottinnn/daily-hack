...
#
# ## t176 が「無い」と言ったのは、**私のチェックが間違っていたから**
#
# `@anthropic-ai/sdk` の `exports` マップは **`./package.json` を公開していない。**
# だから **入っていても `require.resolve('<pkg>/package.json')` は例外で落ちる。**
#
#   require.resolve('@anthropic-ai/sdk/package.json')  → **落ちる**
#   require.resolve('@anthropic-ai/sdk')               → **解決する**
#
# **t173 の「入れられなかった」も同じ偽陰性。** npm も PATH も無関係だった。
#
...
if [ -d "$REPO" ]; then
  if ( cd "$REPO" && node -e "require.resolve('@anthropic-ai/sdk')" >/dev/null 2>&1 ); then
