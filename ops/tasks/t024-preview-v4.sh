...
OUT="${OPS_REPORT_DIR:-/tmp}/preview-v4.md"
DEST="$W/ops"
...
fs.writeFileSync("/tmp/.t024c.json", JSON.stringify(out));
console.log(out.length);
' "$CACHE" "$LIMIT" > /tmp/.t024n 2>/dev/null || echo 0 > /tmp/.t024n
N="$(cat /tmp/.t024n 2>/dev/null || echo 0)"
echo "- 候補: **${N} 件**"
...
i=0
while [ "$i" -lt "$N" ]; do
  TARGET="$("$NODE_BIN" -e 'process.stdout.write(JSON.parse(require("fs").readFileSync("/tmp/.t024c.json","utf8"))[Number(process.argv[1])])' "$i")"
