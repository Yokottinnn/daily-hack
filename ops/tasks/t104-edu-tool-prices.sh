#!/bin/bash
# **ac.jp メールで無料になるツールの「通常いくらか」を取る（t104）。**
#
# 利用者の指摘（2026-09-20・下書きフィードバック）:
#   「通常幾らかかるかの具体的な記載がほしい」
#   「もっと数ないの？わかる数だけ一覧でちゃんと記載して欲しい」
#
# 記事の表はいま「有料プラン」「年3万円前後」のように曖昧。**実額が要る。**
#
# **料金ページは JS で描くものが多い**（Spotify・YouTube で実証済み）。
# 取れなければ取れないと分かる形で出す（ops-task-runner ルール 8）。
#
# **判断はしない。** 採否はクラウド側。
...
OUT="$RDIR/t104-edu-tool-prices.md"
mkdir -p "$RDIR"
...
WANT = re.compile(r"料金|価格|プラン|pricing|Pro|Plus|Professional|Premium|サブスク")
KEY = re.compile(r"料金|価格|プラン|無料|学生|教育|年額|月額|年間|"
                 r"[0-9][0-9,]*\s*円|[$][0-9][0-9.]*")
...
L = ["# ac.jp で無料になるツールの通常価格（t104・公式ページの実物）", "",
