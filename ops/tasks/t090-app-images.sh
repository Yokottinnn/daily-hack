#!/bin/bash
# **t089 で取り切れなかった 4 件を、検索語を変えて取り直す。**
#
# ## t089 で何が起きたか
#
#   jal        1 件目が航空会社の「JAL」。正解は 3 件目の JAL Wellness & Travel だが
#              **画像は上位 2 件しか落としていない**ので手元に無い
#   sugisapo   1 件目が「Green Ponta Action」。**別アプリ**
#   everypoint / tokyouplus   App Store に該当なし
#
# **検索語を変え、候補を 5 件まで見て、画像は 3 件目まで落とす。**
# それでも出てこないものは、記事から外して数を合わせ直す（数字を嘘にしない）。
#
# ## t088 がなぜ空振りしたか
#
#     🚨 /Users/ny/projects/anta-baka-x/blog/ops/data/walk-poikatsu-apps.json が無い
#
# **同じ PR で足したファイルを、タスクがリポジトリから読もうとした。**
# ops-heartbeat はタスクを走らせる前にクローンを pull しないので、届いていない。
# t067 と同じ踏み方であり、**t068 の冒頭に自分で書いた「リポジトリのファイルに
# 依存しない」を破っていた。** 一覧はタスクの中に焼き込む。
#
...
# **一覧はここに焼き込む。** リポジトリのファイルを読みにいかない。
# t088 はクローンに ops/data/walk-poikatsu-apps.json が届いておらず空振りした。
# t067 と同じ踏み方で、t068 の冒頭に自分で書いた教訓を破っていた。
APPS_JSON = r"""[
 {
  "key": "jal",
  "cat": "mile",
  "label": "JAL Wellness & Travel",
  "term": "JAL Wellness & Travel"
 },
 {
  "key": "sugisapo",
  "cat": "goods",
...
L = ["# 歩いてポイ活 21 アプリの App Store 素材（t090）", "",
...
for a in apps:
    ...
    url = ("https://itunes.apple.com/search?"
           + urllib.parse.urlencode({"term": term, "country": "jp",
                                     "entity": "software", "limit": 5, "lang": "ja_jp"}))
