#!/bin/bash
# **新生成器で 5 件 書かせる（呼び出し形を直した再実行）。投稿はしない。**
#
# ## t020 の結果
#
#   - 候補は `data/trend-cache.json` から取れた（13 件・本文フィールドは `text`）
#   - 候補の出どころは実行時の検知 `scripts/trend-detect.js`（DETECT_OUT）
#   - **生成が全件 落ちた**: `ant.create is not a function`
#
# `anthropic-client.js` の呼び方を**推測で書いた**のが原因。
# **API は一度も呼ばれていないので費用は $0** だった。
#
# 直し方は「推測しない」。**モジュールに実際にある関数を探して使う。**
# 見つからなければ何が export されているかを報告する（次で必ず当たるように）。
# 偽モジュールで検証済み: `messages.create` 形で生成でき、
# 関数が無い形では `export: [nope, alsoNope]` と報告した。
#
# ## t019 で分かったこと
#
...
OUT="${OPS_REPORT_DIR:-/tmp}/preview-v2.md"
DEST="$W/ops"
...
{
echo "# 新生成器で 5 件 書かせる（呼び出し形を直した再実行・$(date '+%Y-%m-%d %H:%M:%S') JST）"
echo
...
fs.writeFileSync("/tmp/.t022cand.json", JSON.stringify(out));
console.log(out.length);
' "$CACHE" "$LIMIT" > /tmp/.t022n 2>/dev/null || echo 0 > /tmp/.t022n
N="$(cat /tmp/.t022n 2>/dev/null || echo 0)"
echo "- 使える候補: **${N} 件**"
...
if [ "${N:-0}" = "0" ]; then
  ...
  rm -f /tmp/.t022cand.json /tmp/.t022n
  exit 0
fi
...
i=0
while [ "$i" -lt "$N" ]; do
  TARGET="$("$NODE_BIN" -e 'process.stdout.write(JSON.parse(require("fs").readFileSync("/tmp/.t022cand.json","utf8"))[Number(process.argv[1])])' "$i")"
  RES="$("$NODE_BIN" -e '
