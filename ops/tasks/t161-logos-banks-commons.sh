#!/bin/bash
# **メガバンク 3 行のロゴを、MediaWiki API で取る（t161）。LLM 不使用・$0。**
#
# `best-online-banks-2026` は**見出しに 5 行が名前だけで並んでいる。**
# 在庫にあるのは楽天銀行と JRE BANK の 2 つだけ（最上位ルール 17）。
#
#   みずほ銀行 / 三菱UFJ銀行 / 三井住友銀行
#
# **三井住友カードのロゴを三井住友銀行のロゴとして使わない。** 別会社である
# （運営会社・別ブランドの流用は最上位ルール 17 で禁止）。在庫の `smbccard.png`
# はカード会社のものなので、銀行の見出しには置けない。
#
# **記事に公式サイトへのリンクが無い**ので、**名前で引ける入口**を使う。
