#!/bin/bash
# **ポイントサイト・クレカ・旅行予約のロゴを取る（t130）。**
#
# docs/logo-inventory.md の A 区分（ドメイン 5 以上）のうち、
# **実際にブランドが並んでいる 3 本**を片付ける。
#   pointsite-comparison-2026      … ポイントサイト 4
#   credit-card-campaign-2026-07   … カード 7
#   summer-travel-timesale-2026    … 旅行予約 4
#
# 取り方の土台は t126 / t128 と同じ
# （3 回 試す・percent-encode・**SVG はそのまま持ち帰る**・UA で再試行）。
# SVG のラスタライズはクラウド側の `scripts/svg-to-png.mjs` が行う。
#
...
OUT="$RDIR/t130-logos-point-card-travel.md"
DIR="$RDIR/logos-pct"
mkdir -p "$DIR"
...
SEARCH = [
    ("jcb",        "JCB",            "JCB logo"),
    ("smbccard",   "三井住友カード", "Sumitomo Mitsui Card logo"),
    ("rakutencard","楽天カード",     "Rakuten Card logo"),
    ("jalan",      "じゃらん",       "Jalan net logo"),
]
APPS = []   # **この回はアプリを取らない**
SITES = [
    ("moppy",     "モッピー",         "https://pc.moppy.jp/"),
    ("hapitas",   "ハピタス",         "https://hapitas.jp/"),
    ("pointi",    "ポイントインカム", "https://pointi.jp/"),
    ("pointtown", "ポイントタウン",   "https://www.pointtown.com/"),
    ("dcard",     "dカード",          "https://dcard.docomo.ne.jp/"),
    ("aeoncard",  "イオンカード",     "https://www.aeon.co.jp/"),
    ("epos",      "エポスカード",     "https://www.eposcard.co.jp/"),
    ("paypaycard","PayPayカード",     "https://www.paypay-card.co.jp/"),
    ("ikyu",      "一休.com",         "https://www.ikyu.com/"),
    ("rakutentra","楽天トラベル",     "https://travel.rakuten.co.jp/"),
    ("yahootra",  "Yahoo!トラベル",   "https://travel.yahoo.co.jp/"),
]
...
man = {"_note": "識別目的で使うロゴ。**商標であり自由ライセンスではない。**"
                "取得日 " + dt.date.today().isoformat() + "（ops/tasks/t128-logos-pct.sh）。"}
t0 = time.time()
