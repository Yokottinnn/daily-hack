#!/bin/bash
# **t124 / t125 で取れなかったぶんを取り直す（t128）。**
#
# 取り方は t126 と同じ（**3 回 まで試す・URL を percent-encode する・
# SVG はそのまま持ち帰る・弾かれたらブラウザ相当の UA で再試行**）。
#
# 取り直す理由:
#   - **JAL Wellness & Travel** … t124 の検索 1 件 目が「JAL」（航空券アプリ）だった。
#     **ID を直に指定して取る**（id 1498726068）
#   - **トリマ** … App Store のアイコンが**「コラボ中」の文字入り期間限定版**だった。
#     公式サイトから取る
#   - **楽天損保** … コモンズ検索が **Rakuten TV** を返した
#   - **ソニー損保** … コモンズ検索が**親会社の Sony** を返した。
#     **運営会社のロゴを商品ブランドのロゴにしない**（最上位ルール 17）
#   - **Looopでんき / 保険スクエアbang!** … ページが HTTPError（UA 弾き）
#   - **レモンガス** … **白いロゴ**を掴んで白地で消えた。別のものを探す
#
...
OUT="$RDIR/t128-logos-retry4.md"
DIR="$RDIR/logos-retry4"
mkdir -p "$DIR"
...
SEARCH = [
    ("rakutensonpo", "楽天損保",   "Rakuten Group logo"),
]
# **App Store は ID を直に指定する。** 検索の 1 件 目が別アプリだった（t124）
APPS = [("jalwellness", "JAL Wellness & Travel", "1498726068")]
SITES = [
    ("sonysonpo", "ソニー損保",        "https://www.sonysonpo.co.jp/"),
    ("rakutens",  "楽天損保",          "https://www.rakuten-sonpo.co.jp/"),
    ("looop",     "Looopでんき",       "https://looop.co.jp/denki/"),
    ("bang",      "保険スクエアbang!", "https://www.bang.co.jp/auto/"),
    ("lemongas",  "レモンガス",        "https://www.lemongas.co.jp/"),
...
man = {"_note": "識別目的で使うロゴ。**商標であり自由ライセンスではない。**"
                "取得日 " + dt.date.today().isoformat() + "（ops/tasks/t128-logos-retry4.sh）。"}
t0 = time.time()
