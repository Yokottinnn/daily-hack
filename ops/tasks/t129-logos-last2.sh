#!/bin/bash
# **最後の 2 件（Looopでんき・保険スクエアbang!）を別の入口から取る（t129）。**
#
# t125・t128 でどちらも `HTTPError`。**ブラウザ相当の UA でも弾かれた。**
# トップページが駄目でも、**別のドメイン・別のパスなら開くことがある**ので、
# 入口を増やして当たる。コモンズも一応 見る。
#
# 取り方の土台は t126 / t128 と同じ（3 回 試す・percent-encode・SVG は持ち帰る）。
# **これで取れなければ、取れない理由をレポートに残して終わりにする。**
# 推測で別のロゴを置くことはしない。
#
...
OUT="$RDIR/t129-logos-last2.md"
DIR="$RDIR/logos-last2"
mkdir -p "$DIR"
...
SEARCH = [
    ("looop", "Looopでんき", "Looop Inc logo"),
]
APPS = []   # **この回はアプリを取らない**
SITES = [
    ("looop_a",  "Looopでんき（会社トップ）",   "https://looop.co.jp/"),
    ("looop_b",  "Looopでんき（サービス）",     "https://looop-denki.com/"),
    ("looop_c",  "Looopでんき（低圧）",         "https://looop-denki.com/low-v/"),
    ("bang_a",   "保険スクエアbang!（トップ）", "https://www.bang.co.jp/"),
    ("bang_b",   "保険スクエアbang!（自動車）", "https://www.bang.co.jp/auto/"),
    ("bang_c",   "保険スクエアbang!（会社）",   "https://www.bang.co.jp/company/"),
]
...
man = {"_note": "識別目的で使うロゴ。**商標であり自由ライセンスではない。**"
                "取得日 " + dt.date.today().isoformat() + "（ops/tasks/t128-logos-last2.sh）。"}
t0 = time.time()
