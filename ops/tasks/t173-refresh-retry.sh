...
# ## t171 はなぜ落ちたか（**課金ゼロ**）
#
# **鍵も選定も通ったのに、`@anthropic-ai/sdk` の import で落ちた。**
# `package.json` に宣言が無かった（**真因**）。`total_usd` は **0 → 0**。
#
#   Error [ERR_MODULE_NOT_FOUND]: Cannot find package '@anthropic-ai/sdk'
#
# **直したうえでの再実行。** `refresh-daily.sh` 側にも
# 「無ければ入れる／入らなければ理由を書いて止まる」関門を足してある。
#
# ## なぜ定時を待たないか
#
...
OUT="$RDIR/t173-refresh-retry.md"
...
# --- 先に依存を入れる（**`git reset --hard` は `node_modules` を更新しない**） ---
if [ -d "$REPO" ]; then
  ( cd "$REPO" && git fetch -q origin main 2>/dev/null || true )
  if node -e "require.resolve('@anthropic-ai/sdk/package.json')" >/dev/null 2>&1 \
     || ( cd "$REPO" && node -e "require.resolve('@anthropic-ai/sdk/package.json')" >/dev/null 2>&1 ); then
    echo "- \`@anthropic-ai/sdk\`: **既に入っている**" >> "$OUT"
  else
    echo "- \`@anthropic-ai/sdk\`: 無いので入れる" >> "$OUT"
    ( cd "$REPO" && npm install --no-audit --no-fund --silent @anthropic-ai/sdk ) >/dev/null 2>&1 || true
    if ( cd "$REPO" && node -e "require.resolve('@anthropic-ai/sdk/package.json')" >/dev/null 2>&1 ); then
      echo "  - ✅ **入った**" >> "$OUT"
    else
      echo "  - ❌ **入らなかった。** この先は走らせても同じところで落ちる" >> "$OUT"
    fi
  fi
  echo "" >> "$OUT"
fi
