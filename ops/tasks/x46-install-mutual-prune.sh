...
echo
echo '```'
if ! "$NODE_BIN" --check "$JS.new" 2>/dev/null; then
  echo "  **構文エラー。置かない。**"
  "$NODE_BIN" --check "$JS.new" 2>&1 | head -5 | sed 's/^/    /'
  rm -f "$JS.new"
  echo '```'
  exit 1
fi
echo "  node --check: OK（$(wc -l < "$JS.new" | tr -d ' ') 行）"
[ -f "$JS" ] && cp "$JS" "$JS.bak-$STAMP" && echo "  既存を退避: $(basename "$JS").bak-$STAMP"
mv "$JS.new" "$JS" && chmod +x "$JS"
echo "  置いた: $JS"
echo '```'
...
if ! cdp_ok; then
  ...
  ( cd "$W" && DRY_RUN=1 MAX_UNFOLLOW=8 INACTIVE_DAYS=30 RATIO=0.20 ABS_MIN=300 GRACE_DAYS=14 \
      OPS_WS="$W" CDP_URL="$CDP" timeout 600 "$NODE_BIN" "$JS" ) 2>&1 \
