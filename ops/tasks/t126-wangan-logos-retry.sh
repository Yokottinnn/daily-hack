#!/bin/bash
# **t123 が取れなかったぶんを取り直す（t126）。**
#
# t123 の結果でわかった穴は 4 つ。ここで全部 塞ぐ。
#   1. **コモンズが 5 件 とも `URLError`。** t122 は同じホストで 3 分 前に成功している。
#      → **3 回 まで間を空けて試す。** 1 回の失敗で諦めない
#   2. **`UnicodeEncodeError`** … URL に日本語が入っていた。→ **パスを percent-encode する**
#   3. **SVG が `UnidentifiedImageError`** … Pillow は SVG を開けない。
#      → **SVG はそのまま保存して持ち帰る。** ラスタライズはクラウド側で Chromium でやる
#      （Mac に何が入っているか分からないので、**Mac 側に変換を期待しない**）
#   4. **`HTTPError`** … 既定の UA を弾くサイトがある。→ **ブラウザ相当の UA で 1 回 だけ再試行**
#
# ついでに **SVG 変換に使えるものが Mac にあるか調べて出す**（docs/mac-environment.md が無いため）。
# **判断はしない。** 採否はクラウド側で目で見て決める。
# **秘密は出さない。** LLM 不使用・$0/回・$0/日・$0/月

set -uo pipefail
RDIR="${OPS_REPORT_DIR:-/tmp}"
OUT="$RDIR/t126-wangan-logos-retry.md"
DIR="$RDIR/logos-wangan2"
mkdir -p "$DIR"
...
SITES = [
    ...
    ("senkyaku",  "豊洲 千客万来",       "https://www.toyosu-senkyakubanrai.jp/"),
]
...
man = {"_note": "識別目的で使うロゴ。**商標であり自由ライセンスではない。**"
                "取得日 " + dt.date.today().isoformat() + "（ops/tasks/t126-wangan-logos-retry.sh）。"}
t0 = time.time()
