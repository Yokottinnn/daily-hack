#!/bin/bash
# **週次レポートが走らせている実体を、リポジトリの版に入れ替える。**（t082 の再実行）
#
# ## t082 はなぜ何もしなかったか
#
# **コピー元が無かった。** Mac のクローンが作業ブランチ `ops/t006-sauna-thread-v3`
# のまま 2 週間 放置されていて、`scripts/weekly-blog-report.py` が存在しなかった。
# t083 で main に戻したので、今度はコピー元が在る。
#
# **タスクは 1 回しか走らないので番号を振り直す**（ops-task-runner ルール 1）。
#
# ## t080 で分かったこと
#
...
OUT="$RDIR/t084-install-weekly-report.md"
mkdir -p "$RDIR"
