#!/bin/bash
# **海外リゾート記事と IKEA 記事のロゴを取る（t127）。**
#
# 取り方は t126 と同じ（**3 回 まで試す・URL を percent-encode する・
# SVG はそのまま持ち帰る・弾かれたらブラウザ相当の UA で再試行**）。
#
# 海外リゾートは**ホテルが 9 ブランド**並んでいるので効く。
# IKEA は**リンク 25 本 が全部 IKEA 1 社**なので、**ロゴは 1 枚 だけ**取る。
# 同じロゴを 20 個 並べてもノイズにしかならない（ららぽーとガイドと同じ判断）。
#
...
OUT="$RDIR/t127-resort-ikea-logos.md"
DIR="$RDIR/logos-resort-ikea"
mkdir -p "$DIR"
...
SEARCH = [
    ("ikea",     "IKEA",             "IKEA logo"),
    ("clubmed",  "クラブメッド",     "Club Med logo"),
    ("pullman",  "Pullman",          "Pullman Hotels logo"),
    ("panpac",   "PARKROYAL",        "Pan Pacific Hotels Group logo"),
    ("vinpearl", "Vinpearl",         "Vinpearl logo"),
]
SITES = [
    ("ayana",     "AYANA Resort Bali",          "https://www.ayana.com/ja/bali/"),
    ("amiana",    "Amiana Resort Nha Trang",    "https://amianaresortsandhotels.com/"),
    ("beresort",  "BE Resort Mactan",           "https://beresortmactan.com/"),
    ("orchid",    "Phuket Orchid Resort",       "https://www.katagroup.com/orchid/"),
    ("alacarte",  "À La Carte Da Nang Beach",   "https://alacartedanangbeach.com/en/"),
    ("pullmandn", "Pullman Danang Beach",       "https://www.pullman-danang.com/"),
]
...
man = {"_note": "識別目的で使うロゴ。**商標であり自由ライセンスではない。**"
                "取得日 " + dt.date.today().isoformat() + "（ops/tasks/t127-resort-ikea-logos.sh）。"}
t0 = time.time()
