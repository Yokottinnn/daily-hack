#!/bin/bash
# **いまの条件で外すべき相手を外す（v4）。パッケージ名を `playwright-core` に直した。費用 $0。**
#
# ## 4 回 連続で落ちていた本当の理由（x16 が実物で確定させた）
#
#   Error: Cannot find module 'playwright'
#
# **`playwright` というパッケージは、このワークスペースに存在しない。**
#
#   無い   ~/.openclaw/workspace/node_modules/playwright
#   有る   ~/.openclaw/workspace/node_modules/playwright-core        ← これ
#   有る   /opt/homebrew/lib/node_modules/playwright                 ← グローバル。解決パスに乗らない
#
...
#
# `x09` / `x10` / `x12` / `x13` は 4 回とも **私がパッケージ名を間違えていた**。
# 「cwd が違う」「node のバージョンが違う」という診断は**どちらも外れ**だった。
# node は `/usr/local/bin/node` (v24) でも `/opt/homebrew/bin/node` (v26) でも、
# **`playwright` は無いのだから同じように落ちる。**
#
# ### 教訓
#
# **動いているスクリプトの `require` 行を最初に読む。** 1 行 見れば済んだ。
#
...
S="$W/scripts"
OUT="${OPS_REPORT_DIR:-/tmp}/unfollow-playwright-core.md"
NODE_BIN="/usr/local/bin/node"   # **command -v node は v26 を拾い playwright が無い。実績のあるスクリプトは全部これを明示している**
STATE="$W/data/reply-followers.json"
CDP="http://127.0.0.1:18810"
MAXN="${FRESH_UNFOLLOW_MAX:-5}"
GRACE_DAYS=3
TMPJS="$S/.x17-unfollow.js"
...
{
echo "# いまの条件で外すべき相手を外す v4（$(date '+%Y-%m-%d %H:%M') JST・費用 \$0）"
echo
echo "> **4 回 連続で落ちていたのは、私がパッケージ名を間違えていたから。**"
echo "> \`playwright\` はこのワークスペースに存在せず、実体は **\`playwright-core\`**。"
echo "> 稼働中の \`post-via-playwright.js\` / \`unfollow-handle.js\` / \`post-comment.js\` は"
echo "> **3 本とも \`require(\"playwright-core\")\`** だった。"
echo "> 「cwd が違う」「node のバージョンが違う」という診断は**どちらも外れ**。"
echo
