#!/bin/bash
# **取得元が未記録のロゴを取り直す・その2（共通ポイントと流通）（t184）。LLM 不使用・$0。**
#
# ## なぜ取り直すか
#
...
#
# **この回で見るのは 11 ブランド**（共通ポイント 4・電子マネー 3・商業施設 4）。
# **`vpoint2.png` は `vpoint.png` と同じ Vポイント**なので、取れたものを両方に当てる。
# ポイントサイト・アンケート系は **t183** で見る。
#
...
OUT="$RDIR/t184-logo-sources-b.md"
DIR="$RDIR/logos-logo-sources-b"
mkdir -p "$DIR"
REPO="${DAILY_HACK_REPO:-${BLOG_REPO:-$HOME/projects/anta-baka-x/blog}}"
PORT="${CDP_PORT:-18810}"

{
  echo "# 取得元が未記録のロゴを取り直す・その2（共通ポイントと流通）（t184・**\$0**）"
  echo ""
...
TARGETS='[["dpoint","dポイント","dポイント ドコモ 公式サイト"],["ponta","Pontaポイント","Ponta ポイント 公式サイト"],["vpoint","Vポイント","Vポイント 公式サイト 青と黄色"],["rakuten","楽天ポイント","楽天ポイント 公式サイト"],["waon","WAON POINT","WAON POINT 公式サイト"],["nanaco","nanaco","nanaco 電子マネー 公式サイト"],["jrepoint","JRE POINT","JRE POINT 公式サイト"],["olive","Olive","Olive 三井住友銀行 公式サイト"],["aeon","イオン","イオン 公式サイト"],["marunouchi","丸の内ポイント","丸の内ポイントアプリ 三菱地所 公式サイト"],["mitsuisp","三井ショッピングパークポイント","三井ショッピングパーク 公式サイト"]]'

SCRIPT="$RDIR/.t184-grab.mjs"
cat > "$SCRIPT" <<'JS'
