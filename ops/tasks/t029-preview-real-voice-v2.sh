#!/bin/bash
# **検査を 3 つ足して、同じ 5 件で書き直させる。投稿しない。**
#
# ## t028 で何が起きたか
#
# 声は戻った。入口フィルタも効いた（5 件中 3 件を LLM 前に見送り＝その分は $0）。
# **だが 2 件目に、既存の検査では拾えない欠陥が 3 つ あった。**
#
#   > 外貨建てで減ってるのか…アタシもドル円の動き気になってたわ。
#   > インデックス軸なら長期で見れば大丈夫よ、焦らなさい😌
#
#   1. **「焦らなさい」は日本語として壊れている**（未然形＋なさい）。人は書かない形
#   2. **「大丈夫よ」は他人の資産への根拠のない断定**。相手の保有比率も期間も知らない
#   3. **😌 はテンプレ 35 件に存在しない絵文字**。絵文字も声の一部
#
# しかも相手は「先月比では数％減にとどまる」と**自分で落ち着いている**のに、
# こちらが宥めにいっていた。噛み合い検査は共有語（外貨建/インデックス）で通していた。
#
# ## 足した検査（`reply-relevance-check.cjs` 4-D / 4-E / 4-F）
#
#   4-D 壊れた活用          … 未然形＋なさい を弾く。連用形＋なさい（キャラの命令形）は通す
#   4-E 根拠のない励まし    … 金融語と同居した「大丈夫／心配ない／必ず戻る」を弾く
#   4-F 範囲外の絵文字      … テンプレが使う 14 種の外を弾く
#
# 手元の検証: **2 件目は 3 つとも弾き、1 件目とテンプレの命令形（T27/T29）は通した。**
#
# ## 費用（承認済みの枠内・2026-09-05）
#
#   推定 $0.003 / 件（Haiku 4.5・system 約 2,700 字・出力 ~150 tok）
#   上限 5 件 = 約 $0.015   ← 安全弁。実績ではない
#   想定 2 件 = 約 $0.006   ← t028 の実績で 5 件中 3 件が入口落ち
#
# ## 何が変わったか
#
...
OUT="${OPS_REPORT_DIR:-/tmp}/preview-real-voice-v2.md"
DEST="$W/ops"
...
{
echo "# 検査を 3 つ足して書き直させる（$(date '+%Y-%m-%d %H:%M:%S') JST）"
echo
...
fs.writeFileSync("/tmp/.t029c.json", JSON.stringify(out));
console.log(out.length);
' "$CACHE" "$LIMIT" > /tmp/.t029n 2>/dev/null || echo 0 > /tmp/.t029n
N="$(cat /tmp/.t029n 2>/dev/null || echo 0)"
...
i=0
while [ "$i" -lt "$N" ]; do
  TARGET="$("$NODE_BIN" -e 'process.stdout.write(JSON.parse(require("fs").readFileSync("/tmp/.t029c.json","utf8"))[Number(process.argv[1])])' "$i")"
