#!/bin/bash
# 掲載順位（GSC）を実測する。**039 の再実行版**（done マーカーが残るため番号を変えている）。
#
# 2026-08-30 に 3 記事へまとめて手を入れた（売上効率ランキング・目次修復・X/YouTube 追加）。
# その前の基準線を取るのが目的。**反映には数日かかるので、今日の数字は「打つ前」に近い。**
#
# 016 が残した診断にはこう出ていた（039 で解決済み。念のため同じ手当てを残す）。
#
...
OUT="$RDIR/seo-rankings-0830.md"
DIAG="$RDIR/seo-rankings-0830-diag.md"
...
if "$PY" "$TMP" --days 28 --min-impressions 1 --top 15 --article-queries 15 --queries-per-article 20 --out "$OUT" 2>"$ERR"; then
  rm -f "$TMP" "$ERR" "$DIAG"
  echo "書き出した: reports/seo-rankings-0830.md（$(wc -l < "$OUT" | tr -d ' ') 行）"
  exit 0
fi
...
echo "取得に失敗。診断を reports/seo-rankings-0830-diag.md に更新: $(tail -1 "$ERR" | scrub | cut -c1-160)"
rm -f "$TMP" "$ERR"
exit 1
