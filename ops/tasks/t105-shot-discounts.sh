#!/bin/bash
# **都度の学割の候補を増やすための裏取り（t105）。**
#
# 利用者の指摘（2026-09-20・下書きフィードバック）:
#   「全体的にもっと個数ふやしてほしい。徹底的にリサーチして増やして」
#
# いま入っているのは 映画館・テーマパーク・本・カラオケ・引越し・Apple だけ。
# **遊園地・水族館・ジム・ボウリング・高速バスを足す。**
#
# 一般料金と学生料金の**両方が載っているページ**を取りたいので、
# 各社のトップから「料金」「チケット」を辿る。
#
# **判断はしない。** 採否はクラウド側。
...
OUT="$RDIR/t105-shot-discounts.md"
mkdir -p "$RDIR"
...
SEEDS = [
    ...
    "https://www.willerexpress.com/",
]
WANT = re.compile(r"料金|チケット|入場|入園|入館|価格|会員|学生|割引|パス")
KEY = re.compile(r"料金|チケット|入場|入園|入館|大学生|学生|一般|高校生|中学生|割引|"
                 r"[0-9][0-9,]*\s*円")
...
for seed in SEEDS:
    ...
    found.update(got)
    # **起点そのものが料金ページ**なので、辿った先だけでなく起点も読む対象に入れる。
    found.setdefault(seed, "（起点そのもの）")
L.append("")

for i, (url, text) in enumerate(sorted(found.items())[:14]):
