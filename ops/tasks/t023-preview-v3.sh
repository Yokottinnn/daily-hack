#!/bin/bash
# **動いている呼び出し方をそのまま読んでから、もう一度 書かせる。投稿はしない。**
#
# ## ここまでの経緯
#
...
OUT="${OPS_REPORT_DIR:-/tmp}/preview-v3.md"
DEST="$W/ops"
...
fs.writeFileSync("/tmp/.t023c.json", JSON.stringify(out));
console.log(out.length);
' "$CACHE" "$LIMIT" > /tmp/.t023n 2>/dev/null || echo 0 > /tmp/.t023n
N="$(cat /tmp/.t023n 2>/dev/null || echo 0)"
echo "- 候補: **${N} 件**"
...
i=0
while [ "$i" -lt "$N" ]; do
  TARGET="$("$NODE_BIN" -e 'process.stdout.write(JSON.parse(require("fs").readFileSync("/tmp/.t023c.json","utf8"))[Number(process.argv[1])])' "$i")"
