...
# **`@` 付きだけを伏せると漏れる**（2026-09-14 に実際に漏らした）。
#
#   due unfollows: 20 → mao_otk_tw,cpaky1,sukesankoba,fxmeitantei,...
#
# ログにはハンドルが**裸で、カンマ区切り**で出ることがある。
# `@` を前提にした置換では 1 つも伏せられない。**2 段構えにする。**
#
#   ① `@` 付き        → `@<伏せ>`
#   ② カンマ区切りの列 → 列ごと `<伏せ×N>`（`a,b,c` の形を丸ごと畳む）
#
# ②は「英数字と `_` だけの語が 2 つ以上 カンマで並ぶ」形だけを対象にする。
# 普通の文章や JSON のキー列を巻き込まないよう、**3 文字 以上の語**に限る。
hide() {
