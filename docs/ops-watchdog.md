...
## 2026-08-22: フォロワーを増やすジョブが 1 つも監視されていなかった

`EXPECTED_JOBS` に入っていたのは `comment-warmup` / `incoming-reply-watcher` /
`gateway` / `node` の 4 つだけで、**フォロー系が 1 つも無かった。**

結果、次の 2 つが誰にも検知されずに止まり続けた。

| ジョブ | 止まった日 | 気づくまで |
| --- | --- | --- |
| `competitor-follower-follow`（能動フォロー） | 2026-07-07（CDP 接続断） | **46 日** |
| `badge-followback`（フォロー返し） | 2026-08-09 | **13 日** |

その間フォロワーは 8/09 の 206 から 8/21 の 207 へ、**12 日で +1**。

### 何を見落としていたか

監視は **「載っているジョブが動いているか」** しか見ていなかった。
`launchctl list` に載っていないジョブは一覧に出てこないので、**視界に入らない。**
「あるべきものが消えていること」は、期待するジョブ名を明示して初めて検知できる。

`EXPECTED_JOBS` にフォロー系 4 つを追加した。以後、消えれば **2 時間以内**に鳴る。

### ジョブを増やしたら、ここにも足す

新しい定期ジョブを載せたら `EXPECTED_JOBS` に足すこと。
**足さなければ、そのジョブは「消えても気づかれないジョブ」になる。**

## 2026-08-22: 「名前を知っているものしか見ていない」をやめる

同じ日に 3 つの見落としが同時に露見した。**すべて根は 1 つで、
私が名前を列挙したものしか監視していなかった**こと。

| 見落とし | 結果 |
| --- | --- |
| `EXPECTED_JOBS` にフォロー系が無かった | 能動フォローが **46 日**、フォロー返しが **13 日** 止まった |
| `SLACK_WEBHOOK_URL` が未登録 | 検知 **87 回**が 1 度も配達されなかった |
| 成果（フォロワー数）を見ていなかった | ジョブが載ったまま CDP 断で 0 件実行、でも警告は出なかった |

`EXPECTED_JOBS` に名前を足すだけでは足りない。**手で保守するリストは、
次にジョブが増えたとき また同じ穴が開く。** 足し忘れたジョブは
「消えても気づかれないジョブ」になる。

### 名前を列挙せずに検知する 3 段構え

**1 段目: `unloaded` — plist はあるのに `launchctl` に載っていないもの**

heartbeat が `~/Library/LaunchAgents/` を走査し、`.plist` のまま
`launchctl list` に無いものを全部出す。無効化の意思表示は `.disabled` /
`.bak.*` へのリネームで行う運用なので、**`.plist` のまま載っていないのは
原則すべて異常。** 私が名前を知らないジョブが壊れても捕まる。

**2 段目: `nogrowth` — 成果そのものが目標ペースを大きく下回っていないか**

ジョブが載っていても実行できていなければ意味がない。実際 7/07〜8/09 の
`competitor-follower-follow` は載ったまま CDP 断で 1 件も実行していない。
**壊れ方を知らなくても、未知のジョブが死んでも、結果で捕まる。**

判定は「増加ゼロなら鳴らす」ではなく **目標ペースとの比**にした。
8/09→8/21 は 12 日で +1 で、ゼロではないが完全な停滞であり、
ゼロ判定はこの形を通してしまう。`FOLLOWER_PACE_MIN_PCT`（既定 25%）を
下回ったら鳴らす。目標達成済み（必要ペースが 0 以下）のときは判定しない。

**判定不能を「増えていない」と混同しない。** ログから日付が読めず
`span_days` が出ないときは判定ごと飛ばす。誤報を出す監視は無視されるようになり、
結局また見落とすため。

**3 段目: `nowebhook` — 警告経路そのものが生きているか**

`SLACK_WEBHOOK_URL` が未登録ならそれ自体を異常として鳴らす。
**鳴らない監視は監視ゼロと同じ。**
...
| ケース | 期待 | 結果 |
| --- | --- | --- |
| 12 日で +1（今日の事故の実データ） | 鳴る | `nogrowth` |
| 12 日で 0 人 | 鳴る | `nogrowth` |
| 3 日で +7（目標ペース超え） | 鳴らない | なし |
| 3 日で +4（ペースの半分） | 鳴らない | なし |
| 7 日で +1（ペースの 1 割） | 鳴る | `nogrowth` |
| 目標達成済み | 鳴らない | なし |
