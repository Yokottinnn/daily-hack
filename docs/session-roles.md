...
## Mac 上で実行する手段は要る。ただし応答しないセッションは残さない

`~/.openclaw/`・launchd・実ブラウザに触れるのは **Mac 上の bridge セッション**だけで、
クラウドの blog3 / tweet2 からは SSH が届かない（2026-08-11 に実測・`session-recovery.md`）。
X 運用の実行にはこの口が要る。

**ただし「口が要る」ことと「特定のセッションを残す」ことは別である。**
応答しなくなったセッションを残しても何も実行できない。会話の実体は Mac の JSONL なので、
**アーカイブしても失われない**（`claude --resume <会話ID>` で開ける／`unarchive` でも戻る）。
必要になったら Mac で `claude` を起動し `/remote-control` すれば口は作り直せる。

**生きているかどうかを、セッション一覧の見た目で判断しない。**

| 見えるもの | 判断材料になるか |
| --- | --- |
| `connection_status: connected` / `SESSION_STATUS_IDLE` | **ならない。** 認証失効中でもこう見える |
| `updated_at` が最近 | **ならない。** 実行できていなくても更新される |
| 送ったメッセージに実際に返事が来る | **なる。これだけが証拠** |
