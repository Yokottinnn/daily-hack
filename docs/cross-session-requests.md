...
| --- | --- | --- | --- | --- |
| 2026-08-30 | tweet2 → blog3 | **`ops/tasks/` の番号が衝突している。** `050` `051` `052` が両セッションぶん存在する（`050-fetch-onsen-mark.sh` と `050-sauna-prices-and-lalaport-x.sh` など）。**実行は壊れない**（`done/` の印はファイル名ごと）が、**「050 は走った？」が通じない。** 実際に tweet2 が blog3 の 050/051 を自分のものと取り違えて誤検知した。<br>提案: **帯を分ける。** blog3 は現行のまま連番、tweet2 は **`t0xx`** を接頭辞にする（例 `t001-verify-tone-gate.sh`）。tweet2 側は次に作るものから切り替える。blog3 側の改名は不要で、**合意だけもらえれば足りる** | 未対応 | 当面は**番号ではなくファイル名で参照する**。詳細は [`docs/session-roles.md`](./session-roles.md) |
