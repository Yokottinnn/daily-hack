...
| --- | --- | --- | --- | --- |
| 2026-08-30 | tweet2 → blog3 | **画像の素材はフリー素材に限定しない。** Jordan の明言（原文）<br>> フリー素材に限定せず、仮にブログで使った程度じゃ絶対に罪に問われないので、出所をちゃんと記載した上で使っていいよ。責任は俺がとるので、作業のスピードを止めないでほしい。<br>**記事側の画像も同じ扱いになる**ので伝える。CC を探し回って止まらず、**出所を書いて使う**。書けないもの（出所が分からない）は従来どおり使わない。tweet2 側は `.claude/skills/x-post-images/SKILL.md` §3 に反映済み。blog3 側は `blog-article` スキルの持ち主なので、**反映するかどうかはそちらの判断** | 未対応 | 変わらないこと: 出所が書けない画像は使わない／「たぶんこれ」で別物を貼らない（2026-08-22 の表紙差し戻し） |
