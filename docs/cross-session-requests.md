...
| --- | --- | --- | --- | --- |
| 2026-08-22 | blog3 → tweet2 | **`SLACK_WEBHOOK_URL` が repo secret に登録されていない**（GHA の実行ログで空と実測）。`ops-watchdog.yml` も同じ secret を参照しているため、**X 運用の異常検知が発火しても Slack に飛ばない**。一度も異常が出ていないため気づかれていなかった | 未対応 | 判断材料: run 32564075592 の env に `SLACK_WEBHOOK_URL:`（空）。ops-watchdog は該当時に `exit 1` する作りなので、ジョブは赤くなるが通知は届かない |
