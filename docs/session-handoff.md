...
### 2026-08-22 — 能動フォローが 7/07 から 46 日間止まっていたことが判明（原因は Chrome CDP 127.0.0.1:18800 の接続断で、意図的な停止ではない）。復旧タスク 014 を投入し、監視の EXPECTED_JOBS にフォロー系 4 件を追加した（#199 / #203）

次のアクション:

- [ ] 014 の結果で competitor-follower-follow と hashtag-follow がロードされたかを jobs 配列の実体で確認する。CDP が落ちていればフォローは 0 件のままなので chrome-cdp の復旧が次。SLACK_WEBHOOK_URL の登録は Jordan の手が 1 回必要
