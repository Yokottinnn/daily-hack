...
### 2026-08-22 — Mac への依頼をセッション経由から ops/tasks 経由に切り替え（接続・認証・会話IDに依存しない）。follower-snapshot と SEO ジョブ復旧のタスクを投入。役割分担(#172)と blog3 宛依頼(#182)を main に反映

次のアクション:

- [ ] heartbeat の tasks と jobs を突き合わせて復旧を確認。成功したら ①会話継続（返信への返信）の実装へ
