---
title: "【2026年6月】PayPay大改悪まとめ — ポイント払い0%・公共料金半減・eKYC必須化で損しない完全対策"
description: "2026年6月2日施行のPayPay改悪を完全整理。ポイント払いの還元0%化、公共料金1.0%→0.5%、交通系ICチャージ付与廃止、eKYC本人確認の必須化、PayPayカードゴールドの+0.5%廃止まで。月の利用額別の損益分岐と、今すぐやるべき3つの対策をアタシが解説する。"
publishDate: 2026-06-05
updatedDate: 2026-06-05
category: ["campaigns", "howto", "comparisons"]
tags: ["PayPay", "PayPay改悪", "PayPayカード", "ポイント還元", "eKYC", "公共料金", "ポイ活", "クレジットカード", "節約", "2026年6月"]
isPR: false
draft: false
featured: true
eyecatchUrl: "/images/paypay-2026-june-revision-guide/eyecatch.jpg"
eyecatchAlt: "PayPay大改悪 2026年6月2日施行 — ポイント払い0%・公共料金半減・交通系IC廃止・eKYC必須化を警告するハッカー子"
author: "hacker-ko"
source_candidate_id: "C053"
references: ["https://coki.jp/article/column/83119/", "https://b4c.jp/poigiken/articles/news/paypay-kaiaku.html", "https://news.yahoo.co.jp/expert/articles/58b0025881a2f9941ebf998018e8c7c88d3b94b3", "https://media.moneyforward.com/articles/10699", "https://www.poitan.jp/archives/163933"]
---

「**いつも通りPayPayで払ってたのに、なんかポイント減ってない？**」── その違和感、気のせいじゃない。**2026年6月2日からPayPayが過去最大級の改悪**を施行した。ポイント払いの還元ゼロ化、公共料金の半減、交通系ICチャージの付与廃止、そして**eKYC（本人確認）を済ませてないと還元そのものがゼロ**。知らずに使い続けると、毎月地味〜に損し続ける。

このページでは、**何がどう変わったのか**を一次情報ベースで全部整理して、**あんたが今すぐやるべき3つの対策**と、**月の利用額別にPayPayカード／楽天カード／JREカードどれが得か**まで、まるっと解説する。3分で「自分は何をすればいいか」が分かるようにした。

<div class="hakkako-says">
<img src="/images/expr-06-shock.png" alt="ハッカー子" />
<p>先に一番大事なやつ言っとくね。<strong>①eKYC（本人確認）は今すぐ済ませる（やってないと還元ゼロ）②公共料金のPayPayカード払いはやめて他カードに移す③ポイントは「払う」んじゃなく貯めて使い道を選ぶ</strong>。この3つだけでも被害は最小化できる。順番に説明するよ。</p>
</div>

## ⚠️ 2026年6月2日 PayPay改悪 5つの変更点

まず全体像。今回の改定で変わったのはこの5つ。**赤字＝改悪**。

| 変更点 | 改定前 | 改定後（6/2〜） | 影響度 |
|:---|:---:|:---:|:---:|
| **ポイント払い分の還元** | 付与あり | **付与対象外（0%）** | 🔴 大 |
| **公共料金・税金の還元** | 1.0%（200円→2pt） | **0.5%（200円→1pt）** | 🔴 大 |
| **交通系IC等チャージ（17社）** | 0.5〜1.0% | **付与対象外（0%）** | 🔴 大 |
| **eKYC（本人確認）** | 任意 | **必須化（未完了は還元ゼロ）** | 🔴 特大 |
| **PayPayカードゴールド +0.5%上乗せ** | 基本1.5% | **基本1.0%**（年100万利用で11,000pt新設） | 🟡 中 |

> 🎯 **要するに**：「PayPayで何でも払えばポイントが貯まる」時代は終わった。**“どこで払うか”を選ばないと損する**フェーズに入ったってこと。


<!-- a8-banners auto-inserted by banner-fill (slot1-after-1st-h2) -->
<div class="affiliate-block">
<div class="a8-banner a8-banner--rectangle"><a href="https://px.a8.net/svt/ejp?a8mat=4B3Y38+10BHGA+3SPO+2T8Z4X" target="_blank" rel="sponsored noopener nofollow"><img src="/images/banners/haruene-denki-300x250.jpg" alt="メディア会員の方必見！電気代コスト削減！★大幅還元実施中★【ハルエネ電気】" width="300" height="250" loading="lazy"></a></div>
<div class="a8-banner a8-banner--rectangle"><a href="https://px.a8.net/svt/ejp?a8mat=4B3Y38+11ICNU+3SPO+ZRALD" target="_blank" rel="sponsored noopener nofollow"><img src="/images/banners/gas-choice-300x250.jpg" alt="当サイト限定　最大30000円現金キャッシュバック！ガス代を今よりお安く！【ガスチョイス】" width="300" height="250" loading="lazy"></a></div>
</div>

## 🔍 各変更点をかみくだいて解説

### ① ポイント払いの還元がゼロに

これまでは、貯まったPayPayポイントで支払っても、その支払額にポイントが付いてた（＝複利的に増える）。6/2以降は**ポイントで払った分は還元対象外**。

たとえば1,000円を「200pt＋現金800円」で払うと、**還元計算されるのは現金800円分だけ**。ポイントを“支払いに溶かす”メリットが薄くなった。

### ② 公共料金・税金が1.0%→0.5%に半減

電気・ガス・水道・税金のPayPayカード払いが**200円につき2pt→1ptに半減**。固定費をPayPayに寄せてた人ほどダメージが大きい。ちなみに**dカードも2026年2月に同様の改悪済み**で、これは業界全体の流れ。

### ③ 交通系IC・他社チャージ（17社）の付与廃止

モバイルSuica／PASMO、au PAY、nanaco、WAONなど**17社へのチャージが付与対象外（0%）**に。「PayPayカードでSuicaチャージしてポイント二重取り」の王道ルートが封じられた。

### ④ eKYC（本人確認）が必須化 ← 一番見落とすと痛い

6/2以降、**本人確認（eKYC）が未完了だと、ポイント付与もPayPayステップの倍率カウントも対象外**＝実質還元ゼロ。アプリの案内に従って数分で終わるので、**まだの人は今日やる**。これだけで“知らずに全損”を防げる。

### ⑤ PayPayカードゴールドの+0.5%上乗せ廃止

ゴールドの「基本1.5%」を支えてた**+0.5%上乗せが廃止→基本1.0%に**。代わりに**年間100万円利用で11,000ptボーナス**（年会費11,000円と実質相殺）が新設。年100万円使わない人は**ゴールドの旨味が薄れた**。

<div class="hakkako-says">
<img src="/images/expr-09-arms-crossed.png" alt="ハッカー子" />
<p>④のeKYCだけは本気で今すぐやって。①〜③は「使い方を変える」話だけど、④は<strong>やってないと“正しく使ってても”還元ゼロ</strong>。アプリ開いて本人確認の案内が出てたら、5分で終わらせときな。</p>
</div>

## 🛠️ 今すぐやるべき対策フロー（3パターン）

被害を最小化する具体策。あんたの状況に合わせてどれかを選べばいい。

<div class="compare-cards">
<div class="compare-card recommended">
<h3>🅰 公共料金は「他カード」に逃がす</h3>
<p class="compare-best"><strong>固定費が多い人はこれが最優先</strong></p>
<p>電気・ガス・水道・税金の支払いを**還元率1.0%以上の他カードに切り替える**。固定費は金額が大きいので、0.5%差でも年間で数千円違う。楽天カード・JREカード等の常時1%カードへ。</p>
</div>
<div class="compare-card">
<h3>🅱 ゴールドの継続可否を見直す</h3>
<p class="compare-best"><strong>年100万円使わないなら要検討</strong></p>
<p>+0.5%が消えた今、**年100万円（月約8.3万円）使わないとゴールドの11,000pt特典で年会費を回収できない**。使わない人は一般カードへの切替・解約も選択肢。</p>
</div>
<div class="compare-card">
<h3>🅲 eKYCを済ませて“通常還元”は死守</h3>
<p class="compare-best"><strong>全員 必須（やらないと全損）</strong></p>
<p>本人確認が未完了だと還元ゼロ。**アプリの案内から数分で完了**。これは対策というより「やってないと損が確定する」最低ライン。今日中に。</p>
</div>
</div>


<!-- a8-banners auto-inserted by banner-fill (slot2-middle) -->
<div class="affiliate-block">
<div class="a8-banner a8-banner--rectangle"><a href="https://px.a8.net/svt/ejp?a8mat=4B3Y38+12P7VE+3SDQ+5ZU29" target="_blank" rel="sponsored noopener nofollow"><img src="/images/banners/gasya-no-mado-300x250.jpg" alt="ガス会社の乗り換えで、ガス料金が安くなる！【ガス屋の窓口】" width="300" height="250" loading="lazy"></a></div>
<div class="a8-banner a8-banner--rectangle"><a href="https://px.a8.net/svt/ejp?a8mat=4B3Y38+152YAI+2W92+NXMIP" target="_blank" rel="sponsored noopener nofollow"><img src="/images/banners/enepi-gas-300x250.jpg" alt="1番安いガス料金をWEBで比較して、お得に利用しよう！【エネピ】" width="300" height="250" loading="lazy"></a></div>
</div>

## 💰 月の利用額別 損益分岐 — PayPayカード vs 楽天 vs JRE

「結局PayPayカードのまま使うべき？乗り換える？」を、**普段使い（店舗・ネットの通常決済）の還元率1.0%は据え置き**な点を踏まえて整理した。差が出るのは**公共料金・チャージ**の部分。

| 月の固定費（公共料金等）をPayPay払い | PayPayカード（改定後） | 楽天カード | JRE CARD（Suica民） |
|:---|:---:|:---:|:---:|
| **公共料金の還元率** | 0.5% | 1.0% | 0.5%〜（Suicaチャージは1.5%） |
| **交通系ICチャージ還元** | 0%（廃止） | 0.5% | **1.5%（モバイルSuica）** |
| **通常決済** | 1.0% | 1.0% | 0.5%〜1.0% |
| **向いてる人** | PayPay経済圏メインで店舗決済中心 | 固定費も含め広く1%確保したい | Suica・JR利用が多い首都圏民 |

> 💡 **読み方**：店舗・ネットの**通常決済1.0%は改定後も健在**なので、「PayPayをやめろ」って話じゃない。**“公共料金とICチャージ”だけ別カードに逃がす**のが一番ラク＆効果的。Suica使う人はJRE CARDのモバイルSuicaチャージ1.5%が刺さる。

<div class="hakkako-says">
<img src="/images/expr-05-smug.png" alt="ハッカー子" />
<p>アタシのおすすめは<strong>「PayPayは店の通常決済1%でそのまま使う＋公共料金とSuicaチャージだけ別カードに分ける」</strong>のハイブリッド。全部乗り換えるより、損してる部分だけ切り出す方が手間も少ない。固定費が月5万あるなら、0.5%差で年3,000円は変わるからね。</p>
</div>

## 📌 改悪後も「損しない」ための最終チェック

<ul class="checklist">
<li>
<div class="checklist-body">
<strong>① eKYC（本人確認）を完了させた</strong>
<p>未完了だと還元ゼロ。アプリの案内から数分。最優先。</p>
</div>
</li>
<li>
<div class="checklist-body">
<strong>② 公共料金・税金の支払いカードを1%以上に変更した</strong>
<p>PayPayカードの0.5%から、楽天カード等の1.0%へ。固定費が大きい人ほど効く。</p>
</div>
</li>
<li>
<div class="checklist-body">
<strong>③ 交通系ICチャージのカードを見直した</strong>
<p>PayPayカードのSuica等チャージは0%に。Suica民はJRE CARD（モバイルSuica1.5%）等へ。</p>
</div>
</li>
<li>
<div class="checklist-body">
<strong>④ ゴールドの年100万円利用見込みを確認した</strong>
<p>年100万円使わないなら、11,000pt特典で年会費を回収できない。継続可否を判断。</p>
</div>
</li>
<li>
<div class="checklist-body">
<strong>⑤ ポイントは「支払いに溶かす」より使い道を選ぶ</strong>
<p>ポイント払い分は還元対象外。運用や商品交換など、損しない使い方を意識。</p>
</div>
</li>
</ul>

## まとめ — PayPay改悪、慌てず「損してる部分だけ」直す

1. **eKYCは今日やる** — 未完了は還元ゼロ。これだけは即対応。
2. **公共料金・税金は1%以上の他カードへ** — 半減した0.5%を放置しない。
3. **交通系ICチャージは別カードへ** — PayPayカードは0%、Suica民はJRE CARDが有力。
4. **ゴールドは年100万円利用が分岐点** — 使わないなら見直し。
5. **通常決済1.0%は健在** — PayPayを全部やめる必要はない。“損してる部分だけ”切り出すのが正解。

<div class="hakkako-says">
<img src="/images/expr-04-cheer.png" alt="ハッカー子" />
<p>改悪って聞くと焦るけど、やることはシンプル。<strong>eKYC済ませて、公共料金とSuicaチャージだけ別カードに分ける。それで被害の9割は止まる</strong>。あんたの固定費、ちゃんと1%以上のカードで払えてるか、この機会に1回だけ見直しとこ。</p>
</div>

> 📌 関連記事: [「年会費無料×高還元」クレジットカード徹底比較 2026](/posts/credit-card-no-annual-fee-comparison-2026/) / [お得施策の『本当の時給』ランキング 2026](/posts/money-hacks-hourly-wage-2026/) / [ネット銀行おすすめ比較 2026](/posts/best-online-banks-2026/)
