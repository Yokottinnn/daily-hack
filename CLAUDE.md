...
3. **画像を実際にスレッドへ貼る。** ファイルパスを書くだけでは足りない。
4. **👍 が付くのを待つ。** 確認前に先へ進めない。
5. 👍 を得たら OpenClaw が投稿する（`@OpenClaw tweet <slug>`）。
...
- **文面だけの草案に 👍 が付いても、それは投稿の許可ではない。** 画像を貼り直して取り直す
- ファイルパスを書いただけの状態は「画像を出した」ことにならない。**スレッドに実物が貼られていること**
- クラウドセッションの Slack コネクタは添付できない。**`ops/tasks` 経由でアップロードする**
  （`ops/tasks/041-upload-draft-images.sh` が実装。トークンは値を出さない）
- 画像が貼れないまま急ぐくらいなら、**投稿しないほうがよい**。時機を逃すのは許容できるが、
  中身を見せずに出すのは許容できない
